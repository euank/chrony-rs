...
unsafe extern "C" fn split_ip6(mut ip: *mut IPAddr, mut dst: *mut uint32_t) {
    ...
    while i < 4 as libc::c_int {
        *dst.offset(i as isize) =
            ((*ip).addr.in6[(i * 4 as libc::c_int + 0 as libc::c_int) as
                                usize] as uint32_t) << 24 as libc::c_int |
                (((*ip).addr.in6[(i * 4 as libc::c_int + 1 as libc::c_int) as
                                     usize] as libc::c_int) <<
                     16 as libc::c_int) as libc::c_uint |
                (((*ip).addr.in6[(i * 4 as libc::c_int + 2 as libc::c_int) as
                                     usize] as libc::c_int) <<
                     8 as libc::c_int) as libc::c_uint |
                (*ip).addr.in6[(i * 4 as libc::c_int + 3 as libc::c_int) as
...
/* ================================================== */
#[inline]
unsafe extern "C" fn get_subnet(mut addr: *mut uint32_t,
                                mut where_0: libc::c_uint) -> uint32_t {
    let mut off: libc::c_int = 0;
    off =
        where_0.wrapping_div(32 as libc::c_int as libc::c_uint) as
            libc::c_int;
    where_0 = where_0.wrapping_rem(32 as libc::c_int as libc::c_uint);
    return ((*addr.offset(off as isize) >>
                 ((32 as libc::c_int - 4 as libc::c_int) as
                      libc::c_uint).wrapping_sub(where_0)) as libc::c_ulong &
                ((1 as libc::c_ulong) <<
                     4 as
...
pub unsafe extern "C" fn ADF_CreateTable() -> ADF_AuthTable {
    ...
    result =
        Malloc(::std::mem::size_of::<ADF_AuthTableInst>() as libc::c_ulong) as
            *mut ADF_AuthTableInst;
...
unsafe extern "C" fn close_node(mut node: *mut TableNode) {
    ...
    if !(*node).extended.is_null() {
        ...
        while (i as libc::c_ulong) < (1 as libc::c_ulong) << 4 as libc::c_int
              {
            child_node =
                &mut *(*node).extended.offset(i as isize) as *mut _TableNode;
...
unsafe extern "C" fn open_node(mut node: *mut TableNode) {
    ...
    if (*node).extended.is_null() {
        (*node).extended =
            Malloc2((1 as libc::c_ulong) << 4 as libc::c_int,
                    ::std::mem::size_of::<_TableNode>() as libc::c_ulong) as
                *mut _TableNode;
        i = 0 as libc::c_int;
        while (i as libc::c_ulong) < (1 as libc::c_ulong) << 4 as libc::c_int
              {
...
/* ================================================== */
unsafe extern "C" fn set_subnet(mut start_node: *mut TableNode,
                                mut ip: *mut uint32_t,
 ...
 -> ADF_Status {
    ...
    if subnet_bits < 0 as libc::c_int ||
           subnet_bits > 32 as libc::c_int * ip_len {
        return ADF_BADSUBNET
    } else {
        if bits_to_go & 4 as libc::c_int - 1 as libc::c_int ==
               0 as libc::c_int {
            while bits_to_go > 0 as libc::c_int {
                subnet = get_subnet(ip, bits_consumed as libc::c_uint);
                if (*node).extended.is_null() { open_node(node); }
                node =
                    &mut *(*node).extended.offset(subnet as isize) as
                        *mut _TableNode;
                bits_to_go -= 4 as libc::c_int;
                bits_consumed += 4 as libc::c_int
            }
            if delete_children != 0 { close_node(node); }
            ...
            subnet =
                get_subnet(ip, bits_consumed as libc::c_uint) &
                    !(N - 1 as libc::c_int) as libc::c_uint;
            if subnet.wrapping_add(N as libc::c_uint) as libc::c_ulong <=
                   (1 as libc::c_ulong) << 4 as libc::c_int {
            } else {
                __assert_fail(b"subnet + N <= TABLE_SIZE\x00" as *const u8 as
                                  *const libc::c_char,
                              b"addrfilt.c\x00" as *const u8 as
                                  *const libc::c_char,
                              204 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 69],
                                                        &[libc::c_char; 69]>(b"ADF_Status set_subnet(TableNode *, uint32_t *, int, int, State, int)\x00")).as_ptr());
            }
            if (*node).extended.is_null() { open_node(node); }
            ...
            while j < N {
                this_node =
                    &mut *(*node).extended.offset(i as isize) as
                        *mut _TableNode;
                if delete_children != 0 { close_node(this_node); }
...
/* ================================================== */
unsafe extern "C" fn set_subnet_(mut table: ADF_AuthTable,
 ...
 -> ADF_Status {
    ...
    match (*ip_addr).family as libc::c_int {
        1 => {
            return set_subnet(&mut (*table).base4, &mut (*ip_addr).addr.in4,
                              1 as libc::c_int, subnet_bits, new_state,
                              delete_children)
        }
        2 => {
            split_ip6(ip_addr, ip6.as_mut_ptr());
            return set_subnet(&mut (*table).base6, ip6.as_mut_ptr(),
                              4 as libc::c_int, subnet_bits, new_state,
                              delete_children)
        }
        0 => {
            /* Apply to both, subnet_bits has to be 0 */
            if subnet_bits != 0 as libc::c_int { return ADF_BADSUBNET }
            memset(ip6.as_mut_ptr() as *mut libc::c_void, 0 as libc::c_int,
                   ::std::mem::size_of::<[uint32_t; 4]>() as libc::c_ulong);
            if set_subnet(&mut (*table).base4, ip6.as_mut_ptr(),
                          1 as libc::c_int, 0 as libc::c_int, new_state,
                          delete_children) as libc::c_uint ==
                   ADF_SUCCESS as libc::c_int as libc::c_uint &&
                   set_subnet(&mut (*table).base6, ip6.as_mut_ptr(),
                              4 as libc::c_int, 0 as libc::c_int, new_state,
                              delete_children) as libc::c_uint ==
                       ADF_SUCCESS as libc::c_int as libc::c_uint {
                return ADF_SUCCESS
            }
...
#[no_mangle]
pub unsafe extern "C" fn ADF_Allow(mut table: ADF_AuthTable,
                                   mut ip: *mut IPAddr,
                                   mut subnet_bits: libc::c_int)
 -> ADF_Status {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn ADF_AllowAll(mut table: ADF_AuthTable,
                                      mut ip: *mut IPAddr,
                                      mut subnet_bits: libc::c_int)
 -> ADF_Status {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn ADF_Deny(mut table: ADF_AuthTable,
                                  mut ip: *mut IPAddr,
                                  mut subnet_bits: libc::c_int)
 -> ADF_Status {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn ADF_DenyAll(mut table: ADF_AuthTable,
                                     mut ip: *mut IPAddr,
                                     mut subnet_bits: libc::c_int)
 -> ADF_Status {
...
/* ================================================== */
unsafe extern "C" fn check_ip_in_node(mut start_node: *mut TableNode,
                                      mut ip: *mut uint32_t) -> libc::c_int {
    let mut subnet: uint32_t = 0;
    ...
    node = start_node;
    loop  {
        if (*node).state as libc::c_uint !=
               AS_PARENT as libc::c_int as libc::c_uint {
            state = (*node).state
        }
        if !(*node).extended.is_null() {
            subnet = get_subnet(ip, bits_consumed as libc::c_uint);
        ...
        2 => {
            __assert_fail(b"0\x00" as *const u8 as *const libc::c_char,
                          ...
                          (*::std::mem::transmute::<&[u8; 46],
                                                    &[libc::c_char; 46]>(b"int check_ip_in_node(TableNode *, uint32_t *)\x00")).as_ptr());
        }
        _ => { }
    }
    return result;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn ADF_IsAllowed(mut table: ADF_AuthTable,
                                       mut ip_addr: *mut IPAddr)
 -> libc::c_int {
    let mut ip6: [uint32_t; 4] = [0; 4];
    match (*ip_addr).family as libc::c_int {
        1 => {
            return check_ip_in_node(&mut (*table).base4,
...
/* ================================================== */
unsafe extern "C" fn is_any_allowed(mut node: *mut TableNode,
                                    mut parent: State) -> libc::c_int {
    let mut state: State = DENY;
    let mut i: libc::c_int = 0;
    state =
        if (*node).state as libc::c_uint !=
        ...
        } else { parent as libc::c_uint } as State;
    if state as libc::c_uint != AS_PARENT as libc::c_int as libc::c_uint {
    } else {
        __assert_fail(b"state != AS_PARENT\x00" as *const u8 as
                          *const libc::c_char,
                      b"addrfilt.c\x00" as *const u8 as *const libc::c_char,
                      376 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 39],
                                                &[libc::c_char; 39]>(b"int is_any_allowed(TableNode *, State)\x00")).as_ptr());
    }
    if !(*node).extended.is_null() {
        i = 0 as libc::c_int;
        while (i as libc::c_ulong) < (1 as libc::c_ulong) << 4 as libc::c_int
              {
            if is_any_allowed(&mut *(*node).extended.offset(i as isize),
                              state) != 0 {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn ADF_IsAnyAllowed(mut table: ADF_AuthTable,
