...
extern "C" {
    ...
    #[no_mangle]
    fn SCH_AddFileHandler(fd_0: libc::c_int, events: libc::c_int,
...
static mut rtc_trim: *mut libc::c_double =
...
static mut system_times: *mut timespec =
    0 as *const timespec as *mut timespec;
...
/* Filename supplied by config file where RTC coefficients are
   stored. */
static mut coefs_file_name: *mut libc::c_char =
...
/* ================================================== */
static mut after_init_hook:
       Option<unsafe extern "C" fn(_: *mut libc::c_void) -> ()> =
    None;
static mut after_init_hook_arg: *mut libc::c_void =
...
unsafe extern "C" fn discard_samples(mut new_first: libc::c_int) {
    ...
    if new_first >= 0 as libc::c_int && new_first < n_samples {
        ...
        __assert_fail(b"new_first >= 0 && new_first < n_samples\x00" as
                          *const u8 as *const libc::c_char,
                      b"rtc_linux.c\x00" as *const u8 as *const libc::c_char,
                      166 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 26],
                                                &[libc::c_char; 26]>(b"void discard_samples(int)\x00")).as_ptr());
    }
    n_to_save = n_samples - new_first;
    memmove(rtc_sec as *mut libc::c_void,
            rtc_sec.offset(new_first as isize) as *const libc::c_void,
            (n_to_save as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<time_t>()
                                                 as libc::c_ulong));
    memmove(rtc_trim as *mut libc::c_void,
            rtc_trim.offset(new_first as isize) as *const libc::c_void,
            (n_to_save as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_double>()
                                                 as libc::c_ulong));
    memmove(system_times as *mut libc::c_void,
            system_times.offset(new_first as isize) as *const libc::c_void,
            (n_to_save as
                 libc::c_ulong).wrapping_mul(::std::mem::size_of::<timespec>()
                                                 as libc::c_ulong));
    n_samples = n_to_save;
}
unsafe extern "C" fn accumulate_sample(mut rtc: time_t,
                                       mut sys: *mut timespec) {
    ...
    if n_samples > 0 as libc::c_int &&
           (*rtc_sec.offset((n_samples - 1 as libc::c_int) as isize) - rtc) as
               libc::c_double >=
        ...
        if 0 as libc::c_int != 0 &&
            ...
            LOG_Message(LOGS_DEBUG,
                        b"RTC samples discarded\x00" as *const u8 as
                            *const libc::c_char);
        }
...
/* The new_sample flag is to indicate whether to adjust the
   ...
   deviation. */
unsafe extern "C" fn run_regression(mut new_sample: libc::c_int,
                                    mut valid: *mut libc::c_int,
                                    mut ref_0: *mut time_t,
                                    mut fast: *mut libc::c_double,
                                    mut slope: *mut libc::c_double) {
    let mut rtc_rel: [libc::c_double; 64] =
        [0.; 64]; /* Relative times on RTC axis */
    let mut offsets: [libc::c_double; 64] =
        [0.; 64]; /* How much the RTC is fast of the system clock */
    ...
    if n_samples > 0 as libc::c_int {
        ...
        while i < n_samples {
            rtc_rel[i as usize] =
                *rtc_trim.offset(i as isize) +
                    (*rtc_sec.offset(i as isize) - rtc_ref) as libc::c_double;
            offsets[i as usize] =
                (rtc_ref - (*system_times.offset(i as isize)).tv_sec) as
                    libc::c_double -
                    1.0e-9f64 *
                        (*system_times.offset(i as isize)).tv_nsec as
                            libc::c_double + rtc_rel[i as usize];
            i += 1
        }
        if RGR_FindBestRobustRegression(rtc_rel.as_mut_ptr(),
                                        offsets.as_mut_ptr(), n_samples,
                                        1.0e-9f64, &mut est_intercept,
                                        &mut est_slope, &mut n_runs,
                                        &mut best_new_start) != 0 {
...
unsafe extern "C" fn slew_samples(mut raw: *mut timespec,
    ...
    i = 0 as libc::c_int;
    while i < n_samples {
        UTI_AdjustTimespec(system_times.offset(i as isize), cooked,
                           system_times.offset(i as isize), &mut delta_time,
                           dfreq, doffset);
        i += 1
    }
    ...
    if 0 as libc::c_int != 0 &&
        ...
        LOG_Message(LOGS_DEBUG,
                    b"dfreq=%.8f doffset=%.6f old_fast=%.6f old_rate=%.3f new_fast=%.6f new_rate=%.3f\x00"
                        as *const u8 as *const libc::c_char, dfreq, doffset,
                    old_seconds_fast, 1.0e6f64 * old_gain_rate,
                    coef_seconds_fast, 1.0e6f64 * coef_gain_rate);
    };
}
/* ================================================== */
...
unsafe extern "C" fn rtc_from_t(mut t: *const time_t) -> *mut tm {
    if rtc_on_utc != 0 { return gmtime(t) } else { return localtime(t) };
}
/* ================================================== */
...
unsafe extern "C" fn t_from_rtc(mut stm: *mut tm) -> time_t {
    ...
    t1 = mktime(&mut temp1);
    tm = if rtc_on_utc != 0 { gmtime(&mut t1) } else { localtime(&mut t1) };
    if tm.is_null() {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"gmtime()/localtime() failed\x00" as *const u8 as
                            *const libc::c_char);
        }
    ...
    if t1 - diff == -(1 as libc::c_int) as libc::c_long {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"Could not convert RTC time\x00" as *const u8 as
                            *const libc::c_char);
        }
    }
    return t1 - diff;
...
/* ================================================== */
unsafe extern "C" fn read_hwclock_file(mut hwclock_file:
                                           *const libc::c_char) {
    let mut in_0: *mut FILE = 0 as *mut FILE;
    let mut line: [libc::c_char; 256] = [0; 256];
    let mut i: libc::c_int = 0;
    if hwclock_file.is_null() ||
           *hwclock_file.offset(0 as libc::c_int as isize) == 0 {
        return
    }
    in_0 =
        UTI_OpenFile(0 as *const libc::c_char, hwclock_file,
    ...
    if i == 3 as libc::c_int &&
        ...
        LOG_Message(LOGS_WARN,
                    b"Could not read RTC LOCAL/UTC setting from %s\x00" as
                        *const u8 as *const libc::c_char, hwclock_file);
    };
}
/* ================================================== */
unsafe extern "C" fn setup_config() {
    if CNF_GetRtcOnUtc() != 0 {
        rtc_on_utc = 1 as libc::c_int
    } else { rtc_on_utc = 0 as libc::c_int }
...
unsafe extern "C" fn read_coefs_from_file() {
    let mut in_0: *mut FILE =
        0 as *mut FILE; /* only gets set true if we succeed */
    if tried_to_load_coefs == 0 {
        ...
        if !coefs_file_name.is_null() &&
               {
                   in_0 =
                       UTI_OpenFile(0 as *const libc::c_char, coefs_file_name,
            ...
            if !(fscanf(in_0,
                        b"%d%ld%lf%lf\x00" as *const u8 as
                            *const libc::c_char,
                     ...
                     4 as libc::c_int) {
                LOG_Message(LOGS_WARN,
                            b"Could not read coefficients from %s\x00" as
                                *const u8 as *const libc::c_char,
                            coefs_file_name);
            }
            fclose(in_0);
...
unsafe extern "C" fn write_coefs_to_file(mut valid: libc::c_int,
                                         mut ref_time: time_t,
 ...
 -> libc::c_int {
    ...
    if out.is_null() { return 2 as libc::c_int }
    /* Gain rate is written out in ppm */
    fprintf(out,
            b"%1d %ld %.6f %.3f\n\x00" as *const u8 as *const libc::c_char,
            valid, ref_time, offset, 1.0e6f64 * rate);
    fclose(out);
    /* Rename the temporary file to the correct location */
    if UTI_RenameTempFile(0 as *const libc::c_char, coefs_file_name,
                          b".tmp\x00" as *const u8 as *const libc::c_char,
...
/* ================================================== */
unsafe extern "C" fn switch_interrupts(mut on_off: libc::c_int)
 -> libc::c_int {
    if ioctl(fd,
             (if on_off != 0 {
                  ((0 as libc::c_uint) <<
                       0 as libc::c_int + 8 as libc::c_int + 8 as libc::c_int
                           + 14 as libc::c_int |
                       (('p' as i32) << 0 as libc::c_int + 8 as libc::c_int)
                           as libc::c_uint |
                       ((0x3 as libc::c_int) << 0 as libc::c_int) as
                           libc::c_uint) |
                      ((0 as libc::c_int) <<
                  ...
                  ((0 as libc::c_uint) <<
                       ...
                       (('p' as i32) << 0 as libc::c_int + 8 as libc::c_int)
                           as libc::c_uint |
                       ((0x4 as libc::c_int) << 0 as libc::c_int) as
                           libc::c_uint) |
                      ((0 as libc::c_int) <<
                           0 as libc::c_int + 8 as libc::c_int +
                               8 as libc::c_int) as libc::c_uint
              }) as libc::c_ulong, 0 as libc::c_int) < 0 as libc::c_int {
        LOG_Message(LOGS_ERR,
                    b"Could not %s RTC interrupt : %s\x00" as *const u8 as
                        *const libc::c_char,
                    if on_off != 0 {
    ...
    if on_off != 0 { skip_interrupts = 1 as libc::c_int }
...
pub unsafe extern "C" fn RTC_Linux_Initialise() -> libc::c_int {
    ...
    if fd < 0 as libc::c_int {
        LOG_Message(LOGS_ERR,
                    b"Could not open RTC device %s : %s\x00" as *const u8 as
                        *const libc::c_char, CNF_GetRtcDevice(),
                    strerror(*__errno_location()));
        return 0 as libc::c_int
    }
    /* Make sure the RTC supports interrupts */
    if switch_interrupts(1 as libc::c_int) == 0 ||
           switch_interrupts(0 as libc::c_int) == 0 {
        close(fd);
        return 0 as libc::c_int
    }
    /* Close on exec */
    UTI_FdSetCloexec(fd);
    rtc_sec =
        Malloc2(64 as libc::c_int as size_t,
    ...
    system_times =
        Malloc2(64 as libc::c_int as size_t,
                ::std::mem::size_of::<timespec>() as libc::c_ulong) as
            *mut timespec;
    ...
    SCH_AddFileHandler(fd, 1 as libc::c_int,
                       Some(read_from_device as
    ...
    logfileid =
        if CNF_GetLogRtc() != 0 {
            LOG_FileOpen(b"rtc\x00" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn set_rtc(mut new_rtc_time: time_t) {
    ...
    status =
        ioctl(fd,
              ((1 as libc::c_uint) <<
                   0 as libc::c_int + 8 as libc::c_int + 8 as libc::c_int +
                       14 as libc::c_int |
                   (('p' as i32) << 0 as libc::c_int + 8 as libc::c_int) as
                       libc::c_uint |
                   ((0xa as libc::c_int) << 0 as libc::c_int) as libc::c_uint)
                  as libc::c_ulong |
    ...
    if status < 0 as libc::c_int {
        LOG_Message(LOGS_ERR,
                    b"Could not set RTC time\x00" as *const u8 as
                        *const libc::c_char);
    };
}
/* ================================================== */
unsafe extern "C" fn handle_initial_trim() {
    ...
    run_regression(1 as libc::c_int, &mut coefs_valid, &mut coef_ref_time,
                   &mut coef_seconds_fast, &mut coef_gain_rate);
    n_samples_since_regression = 0 as libc::c_int;
    ...
    if valid_coefs_from_file != 0 {
        ...
        LOG_Message(LOGS_INFO,
                    b"System clock off from RTC by %f seconds (slew)\x00" as
                        *const u8 as *const libc::c_char, sys_error_now);
    } else {
        LOG_Message(LOGS_WARN,
                    b"No valid rtcfile coefficients\x00" as *const u8 as
                        *const libc::c_char);
    }
    coefs_valid = 0 as libc::c_int;
...
unsafe extern "C" fn handle_relock_after_trim() {
    ...
    run_regression(1 as libc::c_int, &mut valid, &mut ref_0, &mut fast,
                   &mut slope);
    if valid != 0 {
        write_coefs_to_file(1 as libc::c_int, ref_0, fast,
                            saved_coef_gain_rate);
    } else if 0 as libc::c_int != 0 &&
                  log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int
     {
        LOG_Message(LOGS_DEBUG,
                    b"Could not do regression after trim\x00" as *const u8 as
                        *const libc::c_char);
    }
    coefs_valid = 0 as libc::c_int;
    n_samples = 0 as libc::c_int;
...
unsafe extern "C" fn maybe_autotrim() {
    ...
    if operating_mode as libc::c_uint !=
           OM_NORMAL as libc::c_int as libc::c_uint || coefs_valid == 0 ||
           n_samples_since_regression != 0 {
        return
    }
    if autotrim_threshold <= 0.0f64 ||
           fabs(coef_seconds_fast) < autotrim_threshold {
        return
    }
    if REF_GetOurStratum() >= 16 as libc::c_int { return }
    RTC_Linux_Trim();
}
/* ================================================== */
unsafe extern "C" fn process_reading(mut rtc_time: time_t,
                                     mut system_time: *mut timespec) {
    ...
    match operating_mode as libc::c_uint {
        0 => {
            if n_samples_since_regression >= 1 as libc::c_int {
                run_regression(1 as libc::c_int, &mut coefs_valid,
                               &mut coef_ref_time, &mut coef_seconds_fast,
                               &mut coef_gain_rate);
                n_samples_since_regression = 0 as libc::c_int;
                maybe_autotrim();
            }
        ...
        _ => {
            __assert_fail(b"0\x00" as *const u8 as *const libc::c_char,
                          b"rtc_linux.c\x00" as *const u8 as
                              *const libc::c_char,
                          744 as libc::c_int as libc::c_uint,
                          (*::std::mem::transmute::<&[u8; 48],
                                                    &[libc::c_char; 48]>(b"void process_reading(time_t, struct timespec *)\x00")).as_ptr());
        }
    }
    if logfileid != -(1 as libc::c_int) {
        rtc_fast =
            (rtc_time - (*system_time).tv_sec) as libc::c_double -
                1.0e-9f64 * (*system_time).tv_nsec as libc::c_double;
        LOG_FileWrite(logfileid,
                      b"%s %14.6f %1d  %14.6f  %12.3f  %2d  %2d %4d\x00" as
                          *const u8 as *const libc::c_char,
                      UTI_TimeToLogForm((*system_time).tv_sec), rtc_fast,
                      coefs_valid, coef_seconds_fast,
                      coef_gain_rate * 1.0e6f64, n_samples, n_runs,
                      measurement_period);
    };
}
/* ================================================== */
unsafe extern "C" fn read_from_device(mut fd_: libc::c_int,
                                      mut event: libc::c_int,
                                      mut any: *mut libc::c_void) {
    let mut status: libc::c_int = 0;
    let mut data: libc::c_ulong = 0;
    let mut sys_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut rtc_raw: rtc_time =
        rtc_time{tm_sec: 0,
                 tm_min: 0,
                 tm_hour: 0,
    ...
    status =
        read(fd, &mut data as *mut libc::c_ulong as *mut libc::c_void,
     ...
     * ready to read but we couldn't read anything.  Give up. */
        LOG_Message(LOGS_ERR,
                    b"Could not read flags %s : %s\x00" as *const u8 as
                        *const libc::c_char, CNF_GetRtcDevice(),
                    strerror(*__errno_location())); /* Likely to raise error too, but just to be sure... */
        SCH_RemoveFileHandler(fd);
        switch_interrupts(0 as libc::c_int);
        close(fd);
        fd = -(1 as libc::c_int);
    ...
    if data & 0x10 as libc::c_int as libc::c_ulong ==
           0x10 as libc::c_int as libc::c_ulong {
        /* Update interrupt detected */
        /* Read RTC time, sandwiched between two polls of the system clock
       so we can bound any error. */
        SCH_GetLastEventTime(&mut sys_time, 0 as *mut libc::c_double,
                             0 as *mut timespec);
        status =
            ioctl(fd,
                  ((2 as libc::c_uint) <<
                       ...
                       (('p' as i32) << 0 as libc::c_int + 8 as libc::c_int)
                           as libc::c_uint |
                       ((0x9 as libc::c_int) << 0 as libc::c_int) as
        ...
        if status < 0 as libc::c_int {
            LOG_Message(LOGS_ERR,
                        b"Could not read time from %s : %s\x00" as *const u8
                            as *const libc::c_char, CNF_GetRtcDevice(),
                        strerror(*__errno_location()));
    ...
    match operating_mode as libc::c_uint {
        1 => {
            if error != 0 {
                if 0 as libc::c_int != 0 &&
                    ...
                    LOG_Message(LOGS_DEBUG,
                                b"Could not complete initial step due to errors\x00"
                                    as *const u8 as *const libc::c_char);
                }
                operating_mode = OM_NORMAL;
                after_init_hook.expect("non-null function pointer")(after_init_hook_arg);
                switch_interrupts(0 as libc::c_int);
                timeout_id =
                    SCH_AddTimeoutByDelay(measurement_period as
                                              libc::c_double,
                                          Some(measurement_timeout as
                                                   unsafe extern "C" fn(_:
                                                                            *mut libc::c_void)
                                                       -> ()),
                                          0 as *mut libc::c_void)
            ...
            if error != 0 {
                if 0 as libc::c_int != 0 &&
                       log_min_severity as libc::c_int ==
                           LOGS_DEBUG as libc::c_int {
                    LOG_Message(LOGS_DEBUG,
                                b"Could not complete after trim relock due to errors\x00"
                                    as *const u8 as *const libc::c_char);
                }
                operating_mode = OM_NORMAL;
                switch_interrupts(0 as libc::c_int);
        ...
        0 => {
            switch_interrupts(0 as libc::c_int);
            timeout_id =
                SCH_AddTimeoutByDelay(measurement_period as libc::c_double,
                                      Some(measurement_timeout as
                                               unsafe extern "C" fn(_:
                                                                        *mut libc::c_void)
                                                   -> ()),
        ...
        _ => {
            __assert_fail(b"0\x00" as *const u8 as *const libc::c_char,
                          ...
                          (*::std::mem::transmute::<&[u8; 40],
                                                    &[libc::c_char; 40]>(b"void read_from_device(int, int, void *)\x00")).as_ptr());
        }
    };
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn RTC_Linux_TimeInit(mut after_hook:
...
pub unsafe extern "C" fn RTC_Linux_WriteParameters() -> libc::c_int {
    ...
    if coefs_valid != 0 {
        retval =
            write_coefs_to_file(1 as libc::c_int, coef_ref_time,
                                coef_seconds_fast, coef_gain_rate)
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn RTC_Linux_TimePreInit(mut driftfile_time: time_t)
 -> libc::c_int {
    ...
    let mut rtc_raw: rtc_time =
        rtc_time{tm_sec: 0,
                 ...
                 tm_yday: 0,
                 tm_isdst: 0,};
    let mut rtc_raw_retry: rtc_time =
        rtc_time{tm_sec: 0,
                 tm_min: 0,
                 tm_hour: 0,
                 tm_mday: 0,
    ...
    let mut new_sys_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut old_sys_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    coefs_file_name = CNF_GetRtcFile();
    setup_config();
    read_coefs_from_file();
    ...
    loop 
     ...
     during the first read operation. */
        ...
        if !(status >= 0 as libc::c_int &&
                 rtc_raw.tm_sec != rtc_raw_retry.tm_sec) {
            break ;
        }
    }
    /* Read system clock */
        ...
        if rtc_t != -(1 as libc::c_int) as time_t {
         ...
         nearest second) */
            if valid_coefs_from_file != 0 {
                accumulated_error =
                    file_ref_offset +
                        (rtc_t - file_ref_time) as libc::c_double * 1.0e-6f64
                            * file_rate_ppm
            } else { accumulated_error = 0.0f64 }
            /* Correct time */
            new_sys_time.tv_sec = rtc_t;
            /* Average error in the RTC reading */
            new_sys_time.tv_nsec =
                500000000 as libc::c_int as __syscall_slong_t;
            UTI_AddDoubleToTimespec(&mut new_sys_time, -accumulated_error,
                                    &mut new_sys_time);
            if new_sys_time.tv_sec < driftfile_time {
                LOG_Message(LOGS_WARN,
                            b"RTC time before last driftfile modification (ignored)\x00"
                                as *const u8 as *const libc::c_char);
                return 0 as libc::c_int
            }
            sys_offset =
                UTI_DiffTimespecsToDouble(&mut old_sys_time,
                                          &mut new_sys_time);
            /* Set system time only if the step is larger than 1 second */
            if fabs(sys_offset) >= 1.0f64 {
                if LCL_ApplyStepOffset(sys_offset) != 0 {
                    LOG_Message(LOGS_INFO,
                                b"System time set from RTC\x00" as *const u8
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn RTC_Linux_GetReport(mut report: *mut RPT_RTC_Report)
 -> libc::c_int {
    ...
    if n_samples > 1 as libc::c_int {
        (*report).span_seconds =
            (*rtc_sec.offset((n_samples - 1 as libc::c_int) as isize) -
                 *rtc_sec.offset(0 as libc::c_int as isize) +
                 (*rtc_trim.offset((n_samples - 1 as libc::c_int) as isize) -
                      *rtc_trim.offset(0 as libc::c_int as isize)) as
                     libc::c_long) as libc::c_ulong
    } else { (*report).span_seconds = 0 as libc::c_int as libc::c_ulong }
...
pub unsafe extern "C" fn RTC_Linux_Trim() -> libc::c_int {
    ...
    if fabs(coef_seconds_fast) > 1.0f64 {
        LOG_Message(LOGS_INFO,
                    b"RTC wrong by %.3f seconds (step)\x00" as *const u8 as
                        *const libc::c_char, coef_seconds_fast);
       ...
       is terminated during rapid sampling */
        coef_seconds_fast =
            -now.tv_nsec as libc::c_double / 1.0e9f64 + 0.5f64;
