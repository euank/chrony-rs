...
unsafe extern "C" fn handle_slew(
    ...
    if (*clock).valid_coefs != 0 {
        (*clock).frequency /= 1.0f64 - dfreq
    };
}
/* Create a new HW clock instance */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn HCL_CreateInstance(
    mut min_samples: libc::c_int,
    mut max_samples: libc::c_int,
    mut min_separation: libc::c_double,
) -> HCL_Instance {
    let mut clock: HCL_Instance = 0 as *mut HCL_Instance_Record;
    min_samples = if 2 as libc::c_int
    ...
    max_samples = if 2 as libc::c_int
    ...
    clock = Malloc(::std::mem::size_of::<HCL_Instance_Record>() as libc::c_ulong)
        as *mut HCL_Instance_Record;
    (*clock).x_data = Malloc2(
        max_samples as size_t,
        ::std::mem::size_of::<libc::c_double>() as libc::c_ulong,
    ) as *mut libc::c_double;
    (*clock).y_data = Malloc2(
        max_samples as size_t,
        ::std::mem::size_of::<libc::c_double>() as libc::c_ulong,
    ) as *mut libc::c_double;
    *(*clock)
        .x_data
        .offset((max_samples - 1 as libc::c_int) as isize) = 0.0f64;
    *(*clock)
        .y_data
        .offset((max_samples - 1 as libc::c_int) as isize) = 0.0f64;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn HCL_NeedsNewSample(
    mut clock: HCL_Instance,
    mut now: *mut timespec,
) -> libc::c_int {
    if (*clock).n_samples == 0
        || fabs(UTI_DiffTimespecsToDouble(now, &mut (*clock).local_ref)) >= (*clock).min_separation
    {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn HCL_AccumulateSample(
    mut clock: HCL_Instance,
    ...
    if (*clock).n_samples != 0 {
        ...
        local_delta = UTI_DiffTimespecsToDouble(local_ts, &mut (*clock).local_ref) / local_freq;
        if hw_delta <= 0.0f64 || local_delta < (*clock).min_separation / 2.0f64 {
            (*clock).n_samples = 0 as libc::c_int;
            if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int
            {
                LOG_Message(
                    LOGS_DEBUG,
                    b"HW clock reset interval=%f\x00" as *const u8 as *const libc::c_char,
                    local_delta,
                );
            }
        }
    ...
    (*clock).valid_coefs = RGR_FindBestRobustRegression(
        (*clock)
            .x_data
            .offset((*clock).max_samples as isize)
            .offset(-((*clock).n_samples as isize)),
        (*clock)
            .y_data
            .offset((*clock).max_samples as isize)
            .offset(-((*clock).n_samples as isize)),
    ...
    if (*clock).valid_coefs == 0 {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"HW clock needs more samples\x00" as *const u8 as *const libc::c_char,
            );
        }
        return;
    }
    ...
    if (*clock).n_samples > (*clock).min_samples {
        (*clock).n_samples -= if best_start < (*clock).n_samples - (*clock).min_samples {
            best_start
        } else {
            ((*clock).n_samples) - (*clock).min_samples
        }
    }
    ...
    if fabs((*clock).offset) > err || fabs((*clock).frequency - 1.0f64) > 2.0f64 / 3.0f64 {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"HW clock reset\x00" as *const u8 as *const libc::c_char,
            );
        }
        (*clock).n_samples = 0 as libc::c_int;
        (*clock).valid_coefs = 0 as libc::c_int
    ...
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"HW clock samples=%d offset=%e freq=%e raw_freq=%e err=%e ref_diff=%e\x00" as *const u8
                as *const libc::c_char,
            (*clock).n_samples,
            (*clock).offset,
            (*clock).frequency - 1.0f64,
            raw_freq - 1.0f64,
            err,
            UTI_DiffTimespecsToDouble(&mut (*clock).hw_ref, &mut (*clock).local_ref),
        );
    };
}
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn HCL_CookTime(
    mut clock: HCL_Instance,
    mut raw: *mut timespec,
    mut cooked: *mut timespec,
