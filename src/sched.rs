 ...
 **********************************************************************
  ...
  */
   ...
   and +1 if a comes after b */
    ...
    #[no_mangle]
    fn UTI_DiffTimespecs(result: *mut timespec, a: *mut timespec,
                         b: *mut timespec);
 ...
 **********************************************************************
  ...
  */
    ...
    #[no_mangle]
    fn LCL_IsFirstParameterChangeHandler(handler: LCL_ParameterChangeHandler)
     -> libc::c_int;
    /* Routine to invoke notify handlers on an unexpected time jump
   in system clock */
    #[no_mangle]
    fn LCL_NotifyExternalTimeStep(raw: *mut timespec, cooked: *mut timespec,
...
static mut last_select_ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
static mut last_select_ts_raw: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
...
static mut timer_queue: TimerQueueEntry =
    TimerQueueEntry{next:
                        0 as *const _TimerQueueEntry as *mut _TimerQueueEntry,
                    prev:
                        0 as *const _TimerQueueEntry as *mut _TimerQueueEntry,
...
static mut last_class_dispatch: [timespec; 4] =
    [timespec{tv_sec: 0, tv_nsec: 0,}; 4];
/* ================================================== */
...
pub unsafe extern "C" fn SCH_Initialise() {
    file_handlers =
        ARR_CreateInstance(::std::mem::size_of::<FileHandlerEntry>() as
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SCH_AddFileHandler(mut fd: libc::c_int,
    ...
    if initialised != 0 {
        ...
        __assert_fail(b"initialised\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 74],
                                                &[libc::c_char; 74]>(b"void SCH_AddFileHandler(int, int, SCH_FileHandler, SCH_ArbitraryArgument)\x00")).as_ptr());
    }
    if events != 0 {
    } else {
        __assert_fail(b"events\x00" as *const u8 as *const libc::c_char,
                      b"sched.c\x00" as *const u8 as *const libc::c_char,
    ...
    if fd >= 0 as libc::c_int {
        ...
        __assert_fail(b"fd >= 0\x00" as *const u8 as *const libc::c_char,
                      b"sched.c\x00" as *const u8 as *const libc::c_char,
    ...
    if fd >= 1024 as libc::c_int {
        LOG_Message(LOGS_FATAL,
                    b"Too many file descriptors\x00" as *const u8 as
                        *const libc::c_char);
    ...
    ptr =
        ARR_GetElement(file_handlers, fd as libc::c_uint) as
            *mut FileHandlerEntry;
    /* Don't want to allow the same fd to register a handler more than
     once without deleting a previous association - this suggests
     a bug somewhere else in the program. */
    if (*ptr).handler.is_none() {
    } else {
        __assert_fail(b"!ptr->handler\x00" as *const u8 as
                          *const libc::c_char,
                      b"sched.c\x00" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn SCH_RemoveFileHandler(mut fd: libc::c_int) {
    ...
    if initialised != 0 {
        ...
        __assert_fail(b"initialised\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 32],
                                                &[libc::c_char; 32]>(b"void SCH_RemoveFileHandler(int)\x00")).as_ptr());
    }
    ptr =
        ARR_GetElement(file_handlers, fd as libc::c_uint) as
            *mut FileHandlerEntry;
    ...
    if (*ptr).handler.is_some() {
        ...
        __assert_fail(b"ptr->handler\x00" as *const u8 as *const libc::c_char,
                      b"sched.c\x00" as *const u8 as *const libc::c_char,
    ...
    while one_highest_fd > 0 as libc::c_int as libc::c_uint {
        ptr =
            ARR_GetElement(file_handlers,
                           one_highest_fd.wrapping_sub(1 as libc::c_int as
                                                           libc::c_uint)) as
                *mut FileHandlerEntry;
        if (*ptr).handler.is_some() { break ; }
        one_highest_fd = one_highest_fd.wrapping_sub(1)
    };
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SCH_SetFileHandlerEvent(mut fd: libc::c_int,
    ...
    if enable != 0 {
        (*ptr).events |= event
    } else { (*ptr).events &= !event };
}
/* Get the time stamp taken after a file descriptor became ready or a timeout expired */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SCH_GetLastEventTime(mut cooked: *mut timespec,
                                              mut err: *mut libc::c_double,
                                              mut raw: *mut timespec) {
    if !cooked.is_null() {
        *cooked = last_select_ts;
        if !err.is_null() { *err = last_select_ts_err }
    }
    if !raw.is_null() { *raw = last_select_ts_raw };
}
unsafe extern "C" fn allocate_tqe() -> *mut TimerQueueEntry {
    ...
    if tqe_free_list.is_null() {
        new_block =
            Malloc2(32 as libc::c_int as size_t,
        ...
        while i < 32 as libc::c_int {
            ...
            *fresh0 =
                &mut *new_block.offset((i - 1 as libc::c_int) as isize) as
                    *mut TimerQueueEntry;
            i += 1
        }
        let ref mut fresh1 =
            (*new_block.offset(0 as libc::c_int as isize)).next;
        *fresh1 = 0 as *mut _TimerQueueEntry;
        tqe_free_list =
            &mut *new_block.offset((32 as libc::c_int - 1 as libc::c_int) as
                                       isize) as *mut TimerQueueEntry
    }
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SCH_AddTimeout(mut ts: *mut timespec,
 ...
 -> SCH_TimeoutID {
    ...
    while ptr != &mut timer_queue as *mut TimerQueueEntry {
        if UTI_CompareTimespecs(&mut (*new_tqe).ts, &mut (*ptr).ts) ==
               -(1 as libc::c_int) {
            break ;
        }
        ptr = (*ptr).next
...
#[no_mangle]
pub unsafe extern "C" fn SCH_AddTimeoutByDelay(mut delay: libc::c_double,
                                               mut handler:
                                                   SCH_TimeoutHandler,
                                               mut arg: SCH_ArbitraryArgument)
 -> SCH_TimeoutID {
    ...
    if UTI_CompareTimespecs(&mut now, &mut then) > 0 as libc::c_int {
        LOG_Message(LOGS_FATAL,
                    b"Timeout overflow\x00" as *const u8 as
                        *const libc::c_char);
        exit(1 as libc::c_int);
    }
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SCH_AddTimeoutInClass(mut min_delay: libc::c_double,
 ...
 -> SCH_TimeoutID {
    ...
    if (class as libc::c_uint) <
           SCH_NumberOfClasses as libc::c_int as libc::c_uint {
    } else {
    ...
    diff =
        UTI_DiffTimespecsToDouble(&mut now,
                                  &mut *last_class_dispatch.as_mut_ptr().offset(class
                                                                                    as
                                                                                    isize));
    if diff < separation && diff >= 0.0f64 &&
           diff + new_min_delay < separation {
    ...
    if initialised != 0 {
        ...
        __assert_fail(b"initialised\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 38],
                                                &[libc::c_char; 38]>(b"void SCH_RemoveTimeout(SCH_TimeoutID)\x00")).as_ptr());
    }
    if id == 0 { return }
    ptr = timer_queue.next;
    while ptr != &mut timer_queue as *mut TimerQueueEntry {
...
unsafe extern "C" fn dispatch_timeouts(mut now: *mut timespec) {
    ...
    let mut n_entries_on_start: libc::c_int =
        n_timer_queue_entries as libc::c_int;
    loop  {
        LCL_ReadRawTime(now);
        if !(n_timer_queue_entries > 0 as libc::c_int as libc::c_ulong &&
                 UTI_CompareTimespecs(now, &mut (*timer_queue.next).ts) >=
                     0 as libc::c_int) {
            break ;
        }
        ptr = timer_queue.next;
       ...
       more time than was delay of a scheduled timeout. */
        if n_done as libc::c_ulong >
               n_timer_queue_entries.wrapping_mul(4 as libc::c_int as
                                                      libc::c_ulong) &&
               n_done > n_entries_on_start * 4 as libc::c_int {
            LOG_Message(LOGS_FATAL,
                        b"Possible infinite loop in scheduling\x00" as
...
unsafe extern "C" fn dispatch_filehandlers(mut nfd: libc::c_int,
        ...
        if !except_fds.is_null() &&
            ...
            ptr =
                ARR_GetElement(file_handlers, fd as libc::c_uint) as
                    *mut FileHandlerEntry;
            if (*ptr).handler.is_some() {
                (*ptr).handler.expect("non-null function pointer")(fd,
                                                                   4 as
                                                                       libc::c_int,
                                                                   (*ptr).arg);
            }
...
unsafe extern "C" fn handle_slew(mut raw: *mut timespec,
    ...
    if change_type as libc::c_uint !=
       ...
       added from other handlers */
        if LCL_IsFirstParameterChangeHandler(Some(handle_slew as
                                                      unsafe extern "C" fn(_:
                                                                               *mut timespec,
                                                                           _:
                                                                               *mut timespec,
        ...
        while i < SCH_NumberOfClasses as libc::c_int {
            UTI_AddDoubleToTimespec(&mut *last_class_dispatch.as_mut_ptr().offset(i
                                                                                      as
                                                                                      isize),
                                    -doffset,
                                    &mut *last_class_dispatch.as_mut_ptr().offset(i
                                                                                      as
                                                                                      isize));
            i += 1
        }
        UTI_AddDoubleToTimespec(&mut last_select_ts_raw, -doffset,
                                &mut last_select_ts_raw);
    }
    UTI_AdjustTimespec(&mut last_select_ts, cooked, &mut last_select_ts,
                       &mut delta, dfreq, doffset);
}
/* ================================================== */
unsafe extern "C" fn fill_fd_sets(mut read_fds: *mut *mut fd_set,
            ...
            if events & 1 as libc::c_int != 0 {
                if rd.is_null() {
                    ...
                    let fresh6 =
                        (::std::mem::size_of::<fd_set>() as
                             libc::c_ulong).wrapping_div(::std::mem::size_of::<__fd_mask>()
                                                             ...
                                                             libc::c_ulong);
                    let fresh7 =
                        &mut *(*rd).fds_bits.as_mut_ptr().offset(0 as
                                                                     libc::c_int
                                                                     as isize)
                            as *mut __fd_mask;
                    asm!("cld; rep; stosq" : "={cx}" (fresh3), "={di}"
                         ...
                         : "memory" : "volatile");
                    c2rust_asm_casts::AsmCast::cast_out(fresh2, fresh6,
                                                        fresh3);
                    c2rust_asm_casts::AsmCast::cast_out(fresh4, fresh7,
                                                        fresh5);
                }
                (*rd).fds_bits[(i /
                                    (8 as libc::c_int *
            ...
            if events & 2 as libc::c_int != 0 {
                if wr.is_null() {
                    ...
                    let fresh12 =
                        (::std::mem::size_of::<fd_set>() as
                             libc::c_ulong).wrapping_div(::std::mem::size_of::<__fd_mask>()
                                                             ...
                                                             libc::c_ulong);
                    let fresh13 =
                        &mut *(*wr).fds_bits.as_mut_ptr().offset(0 as
                                                                     libc::c_int
                                                                     as isize)
                            as *mut __fd_mask;
                    asm!("cld; rep; stosq" : "={cx}" (fresh9), "={di}"
                         ...
                         : "memory" : "volatile");
                    c2rust_asm_casts::AsmCast::cast_out(fresh8, fresh12,
                                                        fresh9);
                    c2rust_asm_casts::AsmCast::cast_out(fresh10, fresh13,
                                                        fresh11);
                }
                (*wr).fds_bits[(i /
                                    (8 as libc::c_int *
            ...
            if events & 4 as libc::c_int != 0 {
                if ex.is_null() {
                    ...
                    let fresh18 =
                        (::std::mem::size_of::<fd_set>() as
                             libc::c_ulong).wrapping_div(::std::mem::size_of::<__fd_mask>()
                                                             ...
                                                             libc::c_ulong);
                    let fresh19 =
                        &mut *(*ex).fds_bits.as_mut_ptr().offset(0 as
                                                                     libc::c_int
                                                                     as isize)
                            as *mut __fd_mask;
                    asm!("cld; rep; stosq" : "={cx}" (fresh15), "={di}"
                         ...
                         : "memory" : "volatile");
                    c2rust_asm_casts::AsmCast::cast_out(fresh14, fresh18,
                                                        fresh15);
                    c2rust_asm_casts::AsmCast::cast_out(fresh16, fresh19,
                                                        fresh17);
                }
                (*ex).fds_bits[(i /
                                    (8 as libc::c_int *
    ...
    if rd.is_null() { *read_fds = 0 as *mut fd_set }
    if wr.is_null() { *write_fds = 0 as *mut fd_set }
    if ex.is_null() { *except_fds = 0 as *mut fd_set };
}
unsafe extern "C" fn check_current_time(mut prev_raw: *mut timespec,
                                        mut raw: *mut timespec,
                                        mut timeout: libc::c_int,
                                        mut orig_select_tv: *mut timeval,
                                        mut rem_select_tv: *mut timeval)
 -> libc::c_int {
    let mut elapsed_min: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut elapsed_max: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut orig_select_ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut rem_select_ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    ...
    if timeout != 0 {
                       ...
                       (*rem_select_tv).tv_usec != (*orig_select_tv).tv_usec)
     {
        ...
        UTI_DiffTimespecs(&mut elapsed_min, &mut orig_select_ts,
                          &mut rem_select_ts);
        elapsed_max = elapsed_min
    } else {
        if !rem_select_tv.is_null() {
            elapsed_max = orig_select_ts
        } else { UTI_DiffTimespecs(&mut elapsed_max, raw, prev_raw); }
    ...
    if last_select_ts_raw.tv_sec + elapsed_min.tv_sec >
        ...
        LOG_Message(LOGS_WARN,
                    b"Backward time jump detected!\x00" as *const u8 as
                        *const libc::c_char);
    } else if ((*prev_raw).tv_sec + elapsed_max.tv_sec +
                   10 as libc::c_int as libc::c_long) < (*raw).tv_sec {
        LOG_Message(LOGS_WARN,
                    b"Forward time jump detected!\x00" as *const u8 as
                        *const libc::c_char);
...
pub unsafe extern "C" fn SCH_MainLoop() {
    ...
    let mut saved_tv: timeval = timeval{tv_sec: 0, tv_usec: 0,};
    let mut ptv: *mut timeval = 0 as *mut timeval;
    let mut ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut now: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut saved_now: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    ...
    if initialised != 0 {
        ...
        __assert_fail(b"initialised\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*::std::mem::transmute::<&[u8; 24],
                                                &[libc::c_char; 24]>(b"void SCH_MainLoop(void)\x00")).as_ptr());
    }
    while need_to_exit == 0 {
        ...
        if n_timer_queue_entries > 0 as libc::c_int as libc::c_ulong {
            ...
            if ts.tv_sec > 0 as libc::c_int as libc::c_long ||
                ...
                __assert_fail(b"ts.tv_sec > 0 || ts.tv_nsec > 0\x00" as
                                  *const u8 as *const libc::c_char,
       ...
       timeout set, this is clearly ridiculous, so stop the run */
        if ptv.is_null() && p_read_fds.is_null() && p_write_fds.is_null() {
            LOG_Message(LOGS_FATAL,
                        b"Nothing to do\x00" as *const u8 as
                            *const libc::c_char);
            exit(1 as libc::c_int);
        }
        status =
            select(one_highest_fd as libc::c_int, p_read_fds, p_write_fds,
                   p_except_fds, ptv);
        errsv = *__errno_location();
        LCL_ReadRawTime(&mut now);
        LCL_CookTime(&mut now, &mut cooked, &mut err);
        /* Check if the time didn't jump unexpectedly */
        if check_current_time(&mut saved_now, &mut now,
        ...
        if status < 0 as libc::c_int {
            if need_to_exit == 0 && errsv != 4 as libc::c_int {
                LOG_Message(LOGS_FATAL,
                            b"select() failed : %s\x00" as *const u8 as
                                *const libc::c_char, strerror(errsv));
                exit(1 as libc::c_int);
            }
            ...
            __assert_fail(b"ptv\x00" as *const u8 as *const libc::c_char,
                          b"sched.c\x00" as *const u8 as *const libc::c_char,
