...
unsafe extern "C" fn update_offset() {
    ...
    if 0 as libc::c_int != 0 &&
        ...
        LOG_Message(LOGS_DEBUG,
                    b"System clock offset=%e freq=%f\x00" as *const u8 as
                        *const libc::c_char, offset_register, freq);
    };
}
/* ================================================== */
unsafe extern "C" fn read_frequency() -> libc::c_double { return freq; }
/* ================================================== */
...
unsafe extern "C" fn offset_convert(mut raw: *mut timespec,
    ...
    if duration > 1000.0f64 { update_offset(); duration = 0.0f64 }
    *corr = -1.0e-6f64 * freq * duration - offset_register;
    if !err.is_null() { *err = 0.0f64 };
}
/*
  chronyd/chronyc - Programs for keeping computer clocks accurate.
...
pub unsafe extern "C" fn SYS_Null_Initialise() {
    ...
    LOG_Message(LOGS_INFO,
                b"Disabled control of system clock\x00" as *const u8 as
                    *const libc::c_char);
}
/* ================================================== */
#[no_mangle]
