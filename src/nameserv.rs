...
extern "C" {
    ...
    #[no_mangle]
    fn SCK_IPSockAddrToSockaddr(
        ip_sa: *mut IPSockAddr,
        sa: *mut sockaddr,
        sa_length: libc::c_int,
    ) -> libc::c_int;
    #[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn DNS_Name2IPAddress(
    ...
    max_addrs = if max_addrs < 16 as libc::c_int {
        max_addrs
    } else {
        16 as libc::c_int
    };
    ...
    result = getaddrinfo(name, 0 as *const libc::c_char, &mut hints, &mut res);
    if result != 0 {
        return DNS_TryAgain;
    }
    ai = res;
    ...
    while i < max_addrs && !ai.is_null() {
        match (*ai).ai_family {
            2 => {
                if !(address_family != 0 as libc::c_int && address_family != 1 as libc::c_int) {
                    (*ip_addrs.offset(i as isize)).family = 1 as libc::c_int as uint16_t;
                    (*ip_addrs.offset(i as isize)).addr.in4 =
                        ntohl((*((*ai).ai_addr as *mut sockaddr_in)).sin_addr.s_addr);
                    i += 1
                }
            }
            10 => {
                if !(address_family != 0 as libc::c_int && address_family != 2 as libc::c_int) {
                    (*ip_addrs.offset(i as isize)).family = 2 as libc::c_int as uint16_t;
                    memcpy(
                        &mut (*ip_addrs.offset(i as isize)).addr.in6 as *mut [uint8_t; 16]
                            as *mut libc::c_void,
                        &mut (*((*ai).ai_addr as *mut sockaddr_in6))
                            .sin6_addr
                            .__in6_u
                            .__u6_addr8 as *mut [uint8_t; 16]
    ...
    return if max_addrs == 0
        || (*ip_addrs.offset(0 as libc::c_int as isize)).family as libc::c_int != 0 as libc::c_int
    {
        DNS_Success as libc::c_int
    } else {
        DNS_Failure as libc::c_int
    } as DNS_Status;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn DNS_IPAddress2Name(
    mut ip_addr: *mut IPAddr,
    mut name: *mut libc::c_char,
    mut len: libc::c_int,
) -> libc::c_int {
    let mut result: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut in6: sockaddr_in6 = sockaddr_in6 {
        sin6_family: 0,
        sin6_port: 0,
        sin6_flowinfo: 0,
        sin6_addr: in6_addr {
    ...
    let mut ip_saddr: IPSockAddr = IPSockAddr {
        ip_addr: IPAddr {
    ...
    slen = SCK_IPSockAddrToSockaddr(
        &mut ip_saddr,
        &mut in6 as *mut sockaddr_in6 as *mut sockaddr,
        ::std::mem::size_of::<sockaddr_in6>() as libc::c_ulong as libc::c_int,
    ) as socklen_t;
    if getnameinfo(
        &mut in6 as *mut sockaddr_in6 as *mut sockaddr,
        slen,
        hbuf.as_mut_ptr(),
        ::std::mem::size_of::<[libc::c_char; 1025]>() as libc::c_ulong as socklen_t,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn DNS_Reload() {
    __res_init();
}
/* ================================================== */
