...
extern "C" {
    ...
    #[no_mangle]
    fn SCK_ReceiveMessages(
        sock_fd: libc::c_int,
        messages: *mut SCK_Message,
        max_messages: libc::c_int,
        flags: libc::c_int,
    ) -> libc::c_int;
    ...
    **********************************************************************
    ...
    #[no_mangle]
    fn NIO_Linux_SetTimestampSocketOptions(
        sock_fd: libc::c_int,
        client_only: libc::c_int,
        events: *mut libc::c_int,
    ) -> libc::c_int;
    #[no_mangle]
    fn NIO_Linux_ProcessEvent(sock_fd: libc::c_int, event: libc::c_int) -> libc::c_int;
    #[no_mangle]
    fn NIO_Linux_ProcessMessage(
        message: *mut SCK_Message,
        local_addr: *mut NTP_Local_Address,
    ...
    #[no_mangle]
    fn NIO_Linux_RequestTxTimestamp(message: *mut SCK_Message, sock_fd: libc::c_int);
    #[no_mangle]
...
unsafe extern "C" fn open_socket(
    ...
    if SCK_IsFamilySupported(family) == 0 {
        return -(1 as libc::c_int);
    }
    if client_only == 0 {
        CNF_GetBindAddress(family, &mut local_addr.ip_addr);
    } else {
        CNF_GetBindAcquisitionAddress(family, &mut local_addr.ip_addr);
    }
    ...
    if sock_fd < 0 as libc::c_int {
        if client_only == 0 {
            LOG_Message(
                LOGS_ERR,
                b"Could not open NTP socket on %s\x00" as *const u8 as *const libc::c_char,
        ...
        && local_addr.port as libc::c_int > 0 as libc::c_int
    {
        bound_server_sock_fd4 =
            (local_addr.ip_addr.addr.in4 != 0 as libc::c_int as in_addr_t) as libc::c_int
    }
    /* Enable kernel/HW timestamping of packets */
    if NIO_Linux_SetTimestampSocketOptions(sock_fd, client_only, &mut events) == 0 {
...
/* ================================================== */
unsafe extern "C" fn open_separate_client_socket(mut remote_addr: *mut IPSockAddr) -> libc::c_int {
    return open_socket(
...
pub unsafe extern "C" fn NIO_Initialise(mut family: libc::c_int) {
    ...
    if client_port < 0 as libc::c_int {
        client_port = 0 as libc::c_int
    }
    permanent_server_sockets = (server_port == 0
        || separate_client_sockets == 0 && client_port == server_port)
        as libc::c_int;
            ...
            && client_sock_fd6 == -(1 as libc::c_int)
    {
        LOG_Message(
            LOGS_FATAL,
            b"Could not open NTP sockets\x00" as *const u8 as *const libc::c_char,
        );
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn NIO_OpenClientSocket(
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn NIO_OpenServerSocket(
    mut remote_addr: *mut NTP_Remote_Address,
) -> libc::c_int {
    match (*remote_addr).ip_addr.family as libc::c_int {
        1 => {
            if permanent_server_sockets != 0 {
    ...
    if sock_fd == server_sock_fd4 {
        ...
        __assert_fail(
            ...
            (*::std::mem::transmute::<&[u8; 32], &[libc::c_char; 32]>(
                b"void NIO_CloseServerSocket(int)\x00",
            ))
            .as_ptr(),
        );
    };
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn NIO_IsServerSocket(mut sock_fd: libc::c_int) -> libc::c_int {
    return (sock_fd != -(1 as libc::c_int)
        && (sock_fd == server_sock_fd4 || sock_fd == server_sock_fd6)) as libc::c_int;
}
/* Function to check if a server socket is currently open */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn NIO_IsServerSocketOpen() -> libc::c_int {
    return (server_sock_fd4 != -(1 as libc::c_int) || server_sock_fd6 != -(1 as libc::c_int))
        as libc::c_int;
}
/* Function to check if client packets can be sent to a server */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn NIO_IsServerConnectable(
    mut remote_addr: *mut NTP_Remote_Address,
) -> libc::c_int {
    let mut sock_fd: libc::c_int = 0;
...
/* ================================================== */
unsafe extern "C" fn process_message(
    ...
    let mut local_ts: NTP_Local_Timestamp = NTP_Local_Timestamp {
        ts: timespec {
            tv_sec: 0,
            tv_nsec: 0,
        },
    ...
    let mut sched_ts: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    SCH_GetLastEventTime(&mut local_ts.ts, &mut local_ts.err, 0 as *mut timespec);
    ...
    if NIO_Linux_ProcessMessage(message, &mut local_addr, &mut local_ts, event) != 0 {
        return;
    }
    if local_ts.source as libc::c_uint != NTP_TS_DAEMON as libc::c_int as libc::c_uint {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"Updated RX timestamp delay=%.9f tss=%u\x00" as *const u8 as *const libc::c_char,
                UTI_DiffTimespecsToDouble(&mut sched_ts, &mut local_ts.ts),
...
/* Forward prototypes */
/* ================================================== */
unsafe extern "C" fn read_from_socket(
    mut sock_fd: libc::c_int,
    mut event: libc::c_int,
    ...
    let mut messages: [SCK_Message; 4] = [SCK_Message {
    ...
    if NIO_Linux_ProcessEvent(sock_fd, event) != 0 {
        return;
    }
    if event == 4 as libc::c_int {
        flags |= 1 as libc::c_int
    }
    received = SCK_ReceiveMessages(sock_fd, messages.as_mut_ptr(), 4 as libc::c_int, flags);
    ...
    while i < received {
        process_message(
            &mut *messages.as_mut_ptr().offset(i as isize),
            sock_fd,
            event,
        );
        i += 1
...
#[no_mangle]
pub unsafe extern "C" fn NIO_SendPacket(
    ...
    let mut message: SCK_Message = SCK_Message {
        data: 0 as *mut libc::c_void,
        length: 0,
        addr_type: SCK_ADDR_UNSPEC,
        if_index: 0,
    ...
    if (*local_addr).sock_fd == -(1 as libc::c_int) {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"No socket to send to %s\x00" as *const u8 as *const libc::c_char,
    ...
    if NIO_IsServerSocket((*local_addr).sock_fd) != 0 || separate_client_sockets == 0 {
    ...
    if SCK_SendMessage((*local_addr).sock_fd, &mut message, 0 as libc::c_int) == 0 {
        return 0 as libc::c_int;
    }
    return 1 as libc::c_int;
}
