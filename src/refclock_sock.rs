...
unsafe extern "C" fn read_sample(
    ...
    let mut sample: sock_sample = sock_sample {
        tv: timeval {
            tv_sec: 0,
            tv_usec: 0,
        },
    ...
    s = recv(
        sockfd,
        &mut sample as *mut sock_sample as *mut libc::c_void,
    ...
    if s < 0 as libc::c_int {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"Could not read SOCK sample : %s\x00" as *const u8 as *const libc::c_char,
                strerror(*__errno_location()),
            );
        }
        return;
    }
    if s as libc::c_ulong != ::std::mem::size_of::<sock_sample>() as libc::c_ulong {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"Unexpected length of SOCK sample : %d != %ld\x00" as *const u8
                    as *const libc::c_char,
                s,
                ::std::mem::size_of::<sock_sample>() as libc::c_ulong as libc::c_long,
            );
        }
        return;
    }
    if sample.magic != 0x534f434b as libc::c_int {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"Unexpected magic number in SOCK sample : %x != %x\x00" as *const u8
                    as *const libc::c_char,
    ...
    if sample.pulse != 0 {
        ...
        RCL_AddSample(instance, &mut ts, sample.offset, sample.leap);
    };
}
unsafe extern "C" fn sock_initialise(mut instance: RCL_Instance) -> libc::c_int {
    let mut sockfd: libc::c_int = 0;
    let mut path: *mut libc::c_char = 0 as *mut libc::c_char;
    RCL_CheckDriverOptions(instance, 0 as *mut *const libc::c_char);
    path = RCL_GetDriverParameter(instance);
    sockfd = SCK_OpenUnixDatagramSocket(0 as *const libc::c_char, path, 0 as libc::c_int);
    if sockfd < 0 as libc::c_int {
        LOG_Message(
            LOGS_FATAL,
            b"Could not open socket %s\x00" as *const u8 as *const libc::c_char,
            path,
        );
...
#[no_mangle]
pub static mut RCL_SOCK_driver: RefclockDriver = unsafe {
    {
        let mut init = RefclockDriver {
            init: Some(sock_initialise as unsafe extern "C" fn(_: RCL_Instance) -> libc::c_int),
            fini: Some(sock_finalise as unsafe extern "C" fn(_: RCL_Instance) -> ()),
            poll: None,
        };
        init
    }
