...
extern "C" {
    ...
    fn CNF_GetTempComp(
        ...
        T0_0: *mut libc::c_double,
        k0_0: *mut libc::c_double,
        k1_0: *mut libc::c_double,
        k2_0: *mut libc::c_double,
    );
...
static mut points: ARR_Instance = 0 as *const ARR_Instance_Record as *mut ARR_Instance_Record;
unsafe extern "C" fn get_tempcomp(mut temp: libc::c_double) -> libc::c_double {
    ...
    return (temp - (*p1).temp) / ((*p2).temp - (*p1).temp) * ((*p2).comp - (*p1).comp)
        + (*p1).comp;
}
unsafe extern "C" fn read_timeout(mut arg: *mut libc::c_void) {
        ...
        ) == 1 as libc::c_int
    {
        ...
        if fabs(comp) <= 10.0f64 {
            ...
            if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int
            {
                LOG_Message(
                    LOGS_DEBUG,
                    b"tempcomp updated to %f for %f\x00" as *const u8 as *const libc::c_char,
                    comp,
                    temp,
                );
            }
            if logfileid != -(1 as libc::c_int) {
                ...
                LOG_FileWrite(
                    logfileid,
                    b"%s %11.4e %11.4e\x00" as *const u8 as *const libc::c_char,
        ...
        LOG_Message(
            LOGS_WARN,
            b"Could not read temperature from %s\x00" as *const u8 as *const libc::c_char,
            filename,
        );
    }
    if !f.is_null() {
    ...
    timeout_id = SCH_AddTimeoutByDelay(
        update_interval,
        Some(read_timeout as unsafe extern "C" fn(_: *mut libc::c_void) -> ()),
...
unsafe extern "C" fn read_points(mut filename_0: *const libc::c_char) {
    ...
    points = ARR_CreateInstance(::std::mem::size_of::<Point>() as libc::c_ulong as libc::c_uint);
    while !fgets(
        line.as_mut_ptr(),
    ...
    .is_null()
    {
        ...
        LOG_Message(
            LOGS_FATAL,
            b"Could not read tempcomp point from %s\x00" as *const u8 as *const libc::c_char,
            filename_0,
        );
    ...
    if ARR_GetSize(points) < 2 as libc::c_int as libc::c_uint {
        LOG_Message(
            LOGS_FATAL,
            b"Not enough points in %s\x00" as *const u8 as *const libc::c_char,
            filename_0,
        );
        exit(1 as libc::c_int);
    };
...
pub unsafe extern "C" fn TMC_Initialise() {
    ...
    if update_interval <= 0.0f64 {
        update_interval = 1.0f64
    }
    if !point_file.is_null() {
        read_points(point_file);
    }
    logfileid = if CNF_GetLogTempComp() != 0 {
        LOG_FileOpen(
            b"tempcomp\x00" as *const u8 as *const libc::c_char,
            b"   Date (UTC) Time        Temp.       Comp.\x00" as *const u8 as *const libc::c_char,
        )
    } else {
        -(1 as libc::c_int)
    };
