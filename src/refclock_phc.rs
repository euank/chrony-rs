...
extern "C" {
    ...
    **********************************************************************
    ...
    #[no_mangle]
    fn RCL_AddCookedPulse(
        instance: RCL_Instance,
        cooked_time: *mut timespec,
        second: libc::c_double,
        dispersion: libc::c_double,
        raw_correction: libc::c_double,
    ) -> libc::c_int;
    #[no_mangle]
    fn RCL_AddSample(
        instance: RCL_Instance,
    ...
    **********************************************************************
    ...
    #[no_mangle]
    fn SYS_Linux_SetPHCExtTimestamping(
        fd: libc::c_int,
        pin: libc::c_int,
        channel: libc::c_int,
        rising: libc::c_int,
    ...
    #[no_mangle]
    fn SYS_Linux_ReadPHCExtTimestamp(
        fd: libc::c_int,
        phc_ts: *mut timespec,
...
unsafe extern "C" fn phc_initialise(mut instance: RCL_Instance) -> libc::c_int {
    ...
    if phc_fd < 0 as libc::c_int {
        LOG_Message(
            LOGS_FATAL,
            b"Could not open PHC\x00" as *const u8 as *const libc::c_char,
        );
        exit(1 as libc::c_int);
    }
    phc = Malloc(::std::mem::size_of::<phc_instance>() as libc::c_ulong) as *mut phc_instance;
    (*phc).fd = phc_fd;
    (*phc).mode = 0 as libc::c_int;
    (*phc).nocrossts = if !RCL_GetDriverOption(
        instance,
        b"nocrossts\x00" as *const u8 as *const libc::c_char as *mut libc::c_char,
    )
    .is_null()
    {
        1 as libc::c_int
    ...
    (*phc).extpps = if !RCL_GetDriverOption(
        instance,
        b"extpps\x00" as *const u8 as *const libc::c_char as *mut libc::c_char,
    )
    .is_null()
    {
        1 as libc::c_int
    ...
    if (*phc).extpps != 0 {
        s = RCL_GetDriverOption(
            instance,
            b"pin\x00" as *const u8 as *const libc::c_char as *mut libc::c_char,
        );
        (*phc).pin = if !s.is_null() {
            atoi(s)
        } else {
            0 as libc::c_int
        };
        s = RCL_GetDriverOption(
            instance,
            b"channel\x00" as *const u8 as *const libc::c_char as *mut libc::c_char,
        );
        (*phc).channel = if !s.is_null() {
            atoi(s)
        } else {
            0 as libc::c_int
        };
        rising_edge = if !RCL_GetDriverOption(
            instance,
            b"clear\x00" as *const u8 as *const libc::c_char as *mut libc::c_char,
        )
        .is_null()
        {
            0 as libc::c_int
        ...
        (*phc).clock = HCL_CreateInstance(
            0 as libc::c_int,
            16 as libc::c_int,
            UTI_Log2ToDouble(RCL_GetDriverPoll(instance)),
        );
        if SYS_Linux_SetPHCExtTimestamping(
            (*phc).fd,
            (*phc).pin,
            (*phc).channel,
            rising_edge,
        ...
        ) == 0
        {
            LOG_Message(
                LOGS_FATAL,
                b"Could not enable external PHC timestamping\x00" as *const u8
...
unsafe extern "C" fn phc_finalise(mut instance: RCL_Instance) {
    ...
    if (*phc).extpps != 0 {
        ...
        SYS_Linux_SetPHCExtTimestamping(
            (*phc).fd,
            (*phc).pin,
            (*phc).channel,
            0 as libc::c_int,
...
unsafe extern "C" fn read_ext_pulse(
    mut fd: libc::c_int,
    ...
    let mut phc: *mut phc_instance = 0 as *mut phc_instance;
    let mut phc_ts: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    let mut local_ts: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    ...
    phc = RCL_GetDriverData(instance) as *mut phc_instance;
    if SYS_Linux_ReadPHCExtTimestamp((*phc).fd, &mut phc_ts, &mut channel) == 0 {
        return;
    }
    ...
    if HCL_CookTime((*phc).clock, &mut phc_ts, &mut local_ts, &mut local_err) == 0 {
        return;
    }
    RCL_AddCookedPulse(
        instance,
...
unsafe extern "C" fn phc_poll(mut instance: RCL_Instance) -> libc::c_int {
    ...
    let mut sys_ts: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    let mut local_ts: timespec = timespec {
    ...
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"PHC offset: %+.9f err: %.9f\x00" as *const u8 as *const libc::c_char,
            offset,
            phc_err,
        );
    }
    return RCL_AddSample(instance, &mut sys_ts, offset, LEAP_Normal as libc::c_int);
}
#[no_mangle]
pub static mut RCL_PHC_driver: RefclockDriver = unsafe {
    {
        let mut init = RefclockDriver {
            init: Some(phc_initialise as unsafe extern "C" fn(_: RCL_Instance) -> libc::c_int),
            fini: Some(phc_finalise as unsafe extern "C" fn(_: RCL_Instance) -> ()),
            poll: Some(phc_poll as unsafe extern "C" fn(_: RCL_Instance) -> libc::c_int),
        };
        init
    }
};
