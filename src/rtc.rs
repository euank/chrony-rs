 ...
 **********************************************************************
  ...
  */
    ...
    #[no_mangle]
    fn RTC_Linux_TimeInit(after_hook:
...
pub struct C2RustUnnamed {
    ...
    pub time_init: Option<unsafe extern "C" fn(_:
                                                   Option<unsafe extern "C" fn(_:
                                                                                   *mut libc::c_void)
                                                              -> ()>,
                                               _: *mut libc::c_void) -> ()>,
    ...
    pub get_report: Option<unsafe extern "C" fn(_: *mut RPT_RTC_Report)
                               -> libc::c_int>,
...
static mut driver: C2RustUnnamed =
    unsafe {
        {
            let mut init =
                C2RustUnnamed{init:
                                  Some(RTC_Linux_Initialise as
                                           unsafe extern "C" fn()
                                               -> libc::c_int),
                              fini:
                                  Some(RTC_Linux_Finalise as
                                           unsafe extern "C" fn() -> ()),
                              time_pre_init:
                                  Some(RTC_Linux_TimePreInit as
                                           unsafe extern "C" fn(_: time_t)
                                               -> libc::c_int),
                              time_init:
                                  Some(RTC_Linux_TimeInit as
                                           unsafe extern "C" fn(_:
                                               ...
                                               -> ()),
                              start_measurements:
                                  Some(RTC_Linux_StartMeasurements as
                                           unsafe extern "C" fn() -> ()),
                              write_parameters:
                                  Some(RTC_Linux_WriteParameters as
                                           unsafe extern "C" fn()
                                               -> libc::c_int),
                              get_report:
                                  Some(RTC_Linux_GetReport as
                                           unsafe extern "C" fn(_:
                                                                    *mut RPT_RTC_Report)
                                               -> libc::c_int),
                              trim:
                                  Some(RTC_Linux_Trim as
                                           unsafe extern "C" fn()
                                               -> libc::c_int),};
            init
        }
...
unsafe extern "C" fn get_driftfile_time() -> time_t {
    ...
    if drift_file.is_null() { return 0 as libc::c_int as time_t }
    if stat(drift_file, &mut buf) != 0 { return 0 as libc::c_int as time_t }
    ...
    if now.tv_sec < t {
        if LCL_ApplyStepOffset((now.tv_sec - t) as libc::c_double) != 0 {
            LOG_Message(LOGS_INFO,
                        b"System time restored from driftfile\x00" as
...
pub unsafe extern "C" fn RTC_Initialise(mut initial_set: libc::c_int) {
    ...
    if initial_set != 0 {
        ...
        if driver.time_pre_init.is_some() &&
               driver.time_pre_init.expect("non-null function pointer")(driftfile_time)
                   != 0 {
            driver_preinit_ok = 1 as libc::c_int
        } else {
            driver_preinit_ok = 0 as libc::c_int;
            if driftfile_time != 0 { apply_driftfile_time(driftfile_time); }
        }
    }
    ...
    if !file_name.is_null() {
        if CNF_GetRtcSync() != 0 {
            LOG_Message(LOGS_FATAL,
                        b"rtcfile directive cannot be used with rtcsync\x00"
        ...
        if driver.init.is_some() {
            ...
            LOG_Message(LOGS_ERR,
                        b"RTC not supported on this operating system\x00" as
                            *const u8 as *const libc::c_char);
        }
    };
}
...
/* Start the processing to get a single measurement from the real time
   ...
   initialisation. */
#[no_mangle]
pub unsafe extern "C" fn RTC_TimeInit(mut after_hook:
                                          Option<unsafe extern "C" fn(_:
                                                                          *mut libc::c_void)
                                                     -> ()>,
                                      mut anything: *mut libc::c_void) {
    if driver_initialised != 0 && driver_preinit_ok != 0 {
        driver.time_init.expect("non-null function pointer")(after_hook,
                                                             anything);
    } else { after_hook.expect("non-null function pointer")(anything); };
}
/*
  chronyd/chronyc - Programs for keeping computer clocks accurate.
...
pub unsafe extern "C" fn RTC_StartMeasurements() {
    if driver_initialised != 0 {
        driver.start_measurements.expect("non-null function pointer")();
    };
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn RTC_GetReport(mut report: *mut RPT_RTC_Report)
 -> libc::c_int {
    if driver_initialised != 0 {
