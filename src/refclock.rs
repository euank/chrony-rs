 ...
 **********************************************************************
  ...
  */
    ...
    fn SPF_AccumulateSample(filter: SPF_Instance, sample: *mut NTP_Sample)
     -> libc::c_int;
    #[no_mangle]
    fn SPF_GetLastSample(filter: SPF_Instance, sample: *mut NTP_Sample)
     -> libc::c_int;
    #[no_mangle]
...
static mut refclocks: ARR_Instance =
...
pub unsafe extern "C" fn RCL_Initialise() {
    refclocks =
        ARR_CreateInstance(::std::mem::size_of::<RCL_Instance>() as
    ...
    logfileid =
        if CNF_GetLogRefclocks() != 0 {
            LOG_FileOpen(b"refclocks\x00" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn RCL_Finalise() {
    ...
    if ARR_GetSize(refclocks) > 0 as libc::c_int as libc::c_uint {
        LCL_RemoveParameterChangeHandler(Some(slew_samples as
                                                  unsafe extern "C" fn(_:
                                                                           *mut timespec,
                                                                       _:
                                                                           *mut timespec,
    ...
    *fresh0 = inst;
    if strcmp((*params).driver_name,
        ...
        LOG_Message(LOGS_FATAL,
                    b"unknown refclock driver %s\x00" as *const u8 as
                        *const libc::c_char, (*params).driver_name);
        exit(1 as libc::c_int);
    }
    if (*(*inst).driver).init.is_none() && (*(*inst).driver).poll.is_none() {
        LOG_Message(LOGS_FATAL,
                    b"refclock driver %s is not compiled in\x00" as *const u8
                        as *const libc::c_char, (*params).driver_name);
        exit(1 as libc::c_int);
    }
    if (*params).tai != 0 && CNF_GetLeapSecTimezone().is_null() {
        LOG_Message(LOGS_FATAL,
                    b"refclock tai option requires leapsectz\x00" as *const u8
    ...
    if !(*inst).driver_parameter.is_null() {
        ...
        (*inst).driver_parameter_length =
            strlen((*inst).driver_parameter) as libc::c_int;
        i = 0 as libc::c_int;
        while i < (*inst).driver_parameter_length {
            if *(*inst).driver_parameter.offset(i as isize) as libc::c_int ==
                   ':' as i32 {
                *(*inst).driver_parameter.offset(i as isize) =
    ...
    if (*params).ref_id != 0 {
        ...
        let mut index: libc::c_uint =
            ARR_GetSize(refclocks).wrapping_sub(1 as libc::c_int as
                                                    libc::c_uint);
        snprintf(ref_0.as_mut_ptr() as *mut libc::c_char,
                 ::std::mem::size_of::<[libc::c_uchar; 5]>() as libc::c_ulong,
                 b"%3.3s\x00" as *const u8 as *const libc::c_char,
                 (*params).driver_name);
        ref_0[3 as libc::c_int as usize] =
            index.wrapping_rem(10 as libc::c_int as
                                   libc::c_uint).wrapping_add('0' as i32 as
                                                                  libc::c_uint)
                as libc::c_uchar;
        if index >= 10 as libc::c_int as libc::c_uint {
            ref_0[2 as libc::c_int as usize] =
                index.wrapping_div(10 as libc::c_int as
                                       libc::c_uint).wrapping_rem(10 as
                                                                      libc::c_int
                                                                      as
    ...
    if (*(*inst).driver).poll.is_some() {
        ...
        max_samples =
            (1 as libc::c_int) << (*inst).poll - (*inst).driver_poll;
        if max_samples < (*params).filter_length {
            if max_samples < 4 as libc::c_int {
                LOG_Message(LOGS_WARN,
                            b"Setting filter length for %s to %d\x00" as
    ...
    if (*(*inst).driver).init.is_some() &&
           (*(*inst).driver).init.expect("non-null function pointer")(inst) ==
               0 {
        LOG_Message(LOGS_FATAL,
                    b"refclock %s initialisation failed\x00" as *const u8 as
                        *const libc::c_char, (*params).driver_name);
        exit(1 as libc::c_int);
    }
    ...
    (*inst).filter =
        SPF_CreateInstance(if (*params).filter_length < 4 as libc::c_int {
                               (*params).filter_length
                           } else { 4 as libc::c_int },
                           (*params).filter_length, (*params).max_dispersion,
                           0.6f64);
    (*inst).source =
        SRC_CreateNewInstance((*inst).ref_id, SRC_REFCLOCK,
                              (*params).sel_options, 0 as *mut IPAddr,
                              (*params).min_samples, (*params).max_samples,
                              0.0f64, 0.0f64);
    if 0 as libc::c_int != 0 &&
           log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(LOGS_DEBUG,
                    b"refclock %s refid=%s poll=%d dpoll=%d filter=%d\x00" as
                        *const u8 as *const libc::c_char,
                    (*params).driver_name, UTI_RefidToString((*inst).ref_id),
                    (*inst).poll, (*inst).driver_poll,
                    (*params).filter_length);
    }
...
pub unsafe extern "C" fn RCL_StartRefclocks() {
    ...
    while i < n {
        ...
        (*inst).timeout_id =
            SCH_AddTimeoutByDelay(0.0f64,
                                  Some(poll_timeout as
                                           unsafe extern "C" fn(_:
                                                                    *mut libc::c_void)
                                               -> ()),
        ...
        if (*inst).lock_ref != 0 {
            ...
            (*inst).lock_ref =
                if j < n { j } else { -(1 as libc::c_int) as libc::c_uint }
        } else { (*inst).lock_ref = -(1 as libc::c_int) as uint32_t }
        i = i.wrapping_add(1)
    };
}
#[no_mangle]
pub unsafe extern "C" fn RCL_ReportSource(mut report: *mut RPT_SourceReport,
                                          mut now: *mut timespec) {
    let mut i: libc::c_uint = 0;
    let mut ref_id: uint32_t = 0;
    if (*report).ip_addr.family as libc::c_int == 1 as libc::c_int {
    } else {
        __assert_fail(b"report->ip_addr.family == IPADDR_INET4\x00" as
                          *const u8 as *const libc::c_char,
                      b"refclock.c\x00" as *const u8 as *const libc::c_char,
                      297 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 61],
                                                &[libc::c_char; 61]>(b"void RCL_ReportSource(RPT_SourceReport *, struct timespec *)\x00")).as_ptr());
    }
...
#[no_mangle]
pub unsafe extern "C" fn RCL_SetDriverData(mut instance: RCL_Instance,
                                           mut data: *mut libc::c_void) {
    (*instance).data = data;
}
#[no_mangle]
pub unsafe extern "C" fn RCL_GetDriverData(mut instance: RCL_Instance)
 -> *mut libc::c_void {
    return (*instance).data;
}
#[no_mangle]
pub unsafe extern "C" fn RCL_GetDriverParameter(mut instance: RCL_Instance)
 -> *mut libc::c_char {
    return (*instance).driver_parameter;
}
unsafe extern "C" fn get_next_driver_option(mut instance: RCL_Instance,
                                            mut option: *mut libc::c_char)
 -> *mut libc::c_char {
    if option.is_null() { option = (*instance).driver_parameter }
    option =
        option.offset(strlen(option).wrapping_add(1 as libc::c_int as
                                                      libc::c_ulong) as
                          isize);
    if option >=
           (*instance).driver_parameter.offset((*instance).driver_parameter_length
                                                   as isize) {
...
#[no_mangle]
pub unsafe extern "C" fn RCL_CheckDriverOptions(mut instance: RCL_Instance,
                                                mut options:
    ...
    while !option.is_null() {
        ...
        while !options.is_null() && !(*options.offset(i as isize)).is_null() {
            ...
            if strncmp(*options.offset(i as isize), option,
                       len as libc::c_ulong) == 0 &&
                   (*option.offset(len as isize) as libc::c_int == '=' as i32
                        ||
                        *option.offset(len as isize) as libc::c_int ==
                            '\u{0}' as i32) {
                break ;
            }
            i += 1
        ...
        if options.is_null() || (*options.offset(i as isize)).is_null() {
            LOG_Message(LOGS_FATAL,
                        b"Invalid refclock driver option %s\x00" as *const u8
                            as *const libc::c_char, option);
            exit(1 as libc::c_int);
        }
...
#[no_mangle]
pub unsafe extern "C" fn RCL_GetDriverOption(mut instance: RCL_Instance,
...
unsafe extern "C" fn convert_tai_offset(mut sample_time: *mut timespec,
                                        mut offset: *mut libc::c_double)
 -> libc::c_int {
    let mut tai_ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut utc_ts: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    ...
    UTI_AddDoubleToTimespec(&mut tai_ts, -tai_offset as libc::c_double,
                            &mut utc_ts);
    tai_offset = REF_GetTaiOffset(&mut utc_ts);
    if tai_offset == 0 { return 0 as libc::c_int }
    *offset -= tai_offset as libc::c_double;
    return 1 as libc::c_int;
}
unsafe extern "C" fn accumulate_sample(mut instance: RCL_Instance,
 ...
 -> libc::c_int {
    ...
    if (*instance).pps_active != 0 &&
           (*instance).lock_ref == -(1 as libc::c_int) as libc::c_uint {
        sample.stratum = pps_stratum(instance, &mut sample.time)
    } else { sample.stratum = (*instance).stratum }
    return SPF_AccumulateSample((*instance).filter, &mut sample);
}
#[no_mangle]
pub unsafe extern "C" fn RCL_AddSample(mut instance: RCL_Instance,
                                       mut sample_time: *mut timespec,
                                       mut offset: libc::c_double,
                                       mut leap: libc::c_int) -> libc::c_int {
    let mut correction: libc::c_double = 0.;
    let mut dispersion: libc::c_double = 0.;
    let mut cooked_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    ...
    match leap {
        ...
        _ => {
            if 0 as libc::c_int != 0 &&
                ...
                LOG_Message(LOGS_DEBUG,
                            b"refclock sample ignored bad leap %d\x00" as
                                *const u8 as *const libc::c_char, leap);
            }
            return 0 as libc::c_int
        }
    }
    if (*instance).tai != 0 &&
           convert_tai_offset(sample_time, &mut offset) == 0 {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"refclock sample ignored unknown TAI offset\x00" as
                            *const u8 as *const libc::c_char);
        }
        return 0 as libc::c_int
    }
    if accumulate_sample(instance, &mut cooked_time,
                         offset - correction + (*instance).offset, dispersion)
           == 0 {
        return 0 as libc::c_int
    }
    (*instance).pps_active = 0 as libc::c_int;
    log_sample(instance, &mut cooked_time, 0 as libc::c_int, 0 as libc::c_int,
               offset, offset - correction + (*instance).offset, dispersion);
    /* for logging purposes */
    if (*(*instance).driver).poll.is_none() { (*instance).driver_polled += 1 }
    return 1 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn RCL_AddPulse(mut instance: RCL_Instance,
 ...
 -> libc::c_int {
    ...
    return RCL_AddCookedPulse(instance, &mut cooked_time, second, dispersion,
                              correction);
}
unsafe extern "C" fn check_pulse_edge(mut instance: RCL_Instance,
                                      mut offset: libc::c_double,
                                      mut distance: libc::c_double)
 -> libc::c_int {
    let mut max_error: libc::c_double = 0.;
    if (*instance).pulse_width <= 0.0f64 { return 1 as libc::c_int }
    max_error =
        1.0f64 / (*instance).pps_rate as libc::c_double -
    ...
    if fabs(offset) > max_error || distance > max_error {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"refclock pulse ignored offset=%.9f distance=%.9f max_error=%.9f\x00"
...
#[no_mangle]
pub unsafe extern "C" fn RCL_AddCookedPulse(mut instance: RCL_Instance,
 ...
 -> libc::c_int {
    ...
    offset -=
        (offset * rate as libc::c_double) as libc::c_long as libc::c_double /
    ...
    if (*instance).lock_ref != -(1 as libc::c_int) as libc::c_uint {
        ...
        if SPF_GetLastSample((*lock_refclock).filter, &mut ref_sample) == 0 {
            if 0 as libc::c_int != 0 &&
                   log_min_severity as libc::c_int ==
                       LOGS_DEBUG as libc::c_int {
                LOG_Message(LOGS_DEBUG,
                            b"refclock pulse ignored no ref sample\x00" as
                                *const u8 as *const libc::c_char);
            }
            return 0 as libc::c_int
        }
        ref_sample.root_dispersion +=
            SPF_GetAvgSampleDispersion((*lock_refclock).filter);
        sample_diff =
            UTI_DiffTimespecsToDouble(cooked_time, &mut ref_sample.time);
        if fabs(sample_diff) >=
               (*instance).max_lock_age as libc::c_double /
                   rate as libc::c_double {
            if 0 as libc::c_int != 0 &&
                   log_min_severity as libc::c_int ==
                       LOGS_DEBUG as libc::c_int {
                LOG_Message(LOGS_DEBUG,
                            b"refclock pulse ignored samplediff=%.9f\x00" as
                                *const u8 as *const libc::c_char,
                            sample_diff);
            }
            return 0 as libc::c_int
        ...
        if ref_sample.offset - offset >= 0.0f64 {
            shift =
                ((ref_sample.offset - offset) * rate as libc::c_double +
                     0.5f64) as libc::c_long as libc::c_double /
                    rate as libc::c_double
        } else {
            shift =
                ((ref_sample.offset - offset) * rate as libc::c_double -
        ...
        offset += shift;
        if fabs(ref_sample.offset - offset) + ref_sample.root_dispersion +
               dispersion >= 0.2f64 / rate as libc::c_double {
            if 0 as libc::c_int != 0 &&
                   log_min_severity as libc::c_int ==
                       LOGS_DEBUG as libc::c_int {
                LOG_Message(LOGS_DEBUG,
                            b"refclock pulse ignored offdiff=%.9f refdisp=%.9f disp=%.9f\x00"
                                as *const u8 as *const libc::c_char,
                            ref_sample.offset - offset,
                            ref_sample.root_dispersion, dispersion);
            }
            return 0 as libc::c_int
        }
        ...
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"refclock pulse offset=%.9f offdiff=%.9f samplediff=%.9f\x00"
                            as *const u8 as *const libc::c_char, offset,
                        ref_sample.offset - offset, sample_diff);
        }
    } else {
       ...
       reference is not active */
        REF_GetReferenceParams(cooked_time, &mut is_synchronised, &mut leap,
                               &mut stratum, &mut ref_id, &mut ref_time,
                               &mut root_delay, &mut root_dispersion);
        distance =
            fabs(root_delay) / 2 as libc::c_int as libc::c_double +
                root_dispersion;
        if leap as libc::c_uint ==
            ...
            if 0 as libc::c_int != 0 &&
                   log_min_severity as libc::c_int ==
                       LOGS_DEBUG as libc::c_int {
                LOG_Message(LOGS_DEBUG,
                            b"refclock pulse ignored offset=%.9f sync=%d dist=%.9f\x00"
                                as *const u8 as *const libc::c_char, offset,
                            (leap as libc::c_uint !=
                                 LEAP_Unsynchronised as libc::c_int as
                                     libc::c_uint) as libc::c_int, distance);
            }
    ...
    log_sample(instance, cooked_time, 0 as libc::c_int, 1 as libc::c_int,
               offset + raw_correction - (*instance).offset, offset,
               dispersion);
    /* for logging purposes */
    if (*(*instance).driver).poll.is_none() { (*instance).driver_polled += 1 }
    return 1 as libc::c_int;
...
#[no_mangle]
pub unsafe extern "C" fn RCL_GetPrecision(mut instance: RCL_Instance)
 -> libc::c_double {
...
#[no_mangle]
pub unsafe extern "C" fn RCL_GetDriverPoll(mut instance: RCL_Instance)
 -> libc::c_int {
    return (*instance).driver_poll;
}
unsafe extern "C" fn valid_sample_time(mut instance: RCL_Instance,
                                       mut sample_time: *mut timespec)
 -> libc::c_int {
    ...
    if diff < 0.0f64 ||
           diff > UTI_Log2ToDouble((*instance).poll + 1 as libc::c_int) {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"%s refclock sample time %s not valid age=%.6f\x00"
                            as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn pps_stratum(mut instance: RCL_Instance,
                                 mut ts: *mut timespec) -> libc::c_int {
    let mut ref_time: timespec = timespec{tv_sec: 0, tv_nsec: 0,};
    let mut is_synchronised: libc::c_int = 0;
    let mut stratum: libc::c_int = 0;
    let mut i: libc::c_uint = 0;
    ...
    REF_GetReferenceParams(ts, &mut is_synchronised, &mut leap, &mut stratum,
                           &mut ref_id, &mut ref_time, &mut root_delay,
                           &mut root_dispersion);
    ...
    while i < ARR_GetSize(refclocks) {
        ...
        if (*refclock).ref_id == ref_id && (*refclock).pps_active != 0 &&
...
unsafe extern "C" fn poll_timeout(mut arg: *mut libc::c_void) {
    ...
    if !((*(*inst).driver).poll.is_some() &&
        ...
        if SPF_GetFilteredSample((*inst).filter, &mut sample) != 0 {
            ...
            log_sample(inst, &mut sample.time, 1 as libc::c_int,
                       0 as libc::c_int, 0.0f64, sample.offset,
                       sample.peer_dispersion);
        } else { SRC_UpdateReachability((*inst).source, 0 as libc::c_int); }
    }
    (*inst).timeout_id =
        SCH_AddTimeoutByDelay(UTI_Log2ToDouble(poll),
                              Some(poll_timeout as
                                       unsafe extern "C" fn(_:
                                                                *mut libc::c_void)
                                           -> ()), arg);
...
unsafe extern "C" fn slew_samples(mut raw: *mut timespec,
    ...
    while i < ARR_GetSize(refclocks) {
        if change_type as libc::c_uint ==
            ...
            SPF_SlewSamples((*get_refclock(i)).filter, cooked, dfreq,
                            doffset);
        }
        i = i.wrapping_add(1)
    };
...
unsafe extern "C" fn log_sample(mut instance: RCL_Instance,
                                mut sample_time: *mut timespec,
    ...
    if filtered == 0 {
        LOG_FileWrite(logfileid,
                      b"%s.%06d %-5s %3d %1c %1d %13.6e %13.6e %10.3e\x00" as
                          *const u8 as *const libc::c_char,
                      UTI_TimeToLogForm((*sample_time).tv_sec),
                      (*sample_time).tv_nsec as libc::c_int /
                          1000 as libc::c_int,
                      UTI_RefidToString((*instance).ref_id),
                      (*instance).driver_polled,
                      sync_stats[(*instance).leap_status as usize] as
                          libc::c_int, pulse, raw_offset, cooked_offset,
                      dispersion);
    } else {
        LOG_FileWrite(logfileid,
                      b"%s.%06d %-5s   - %1c -       -       %13.6e %10.3e\x00"
                          as *const u8 as *const libc::c_char,
                      UTI_TimeToLogForm((*sample_time).tv_sec),
                      (*sample_time).tv_nsec as libc::c_int /
                          1000 as libc::c_int,
                      UTI_RefidToString((*instance).ref_id),
                      sync_stats[(*instance).leap_status as usize] as
                          libc::c_int, cooked_offset, dispersion);
    };
}
