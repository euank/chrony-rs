...
static mut keys: ARR_Instance = 0 as *const ARR_Instance_Record as *mut ARR_Instance_Record;
...
unsafe extern "C" fn free_keys() {
    ...
    while i < ARR_GetSize(keys) {
        ...
        match (*key).class as libc::c_uint {
            0 => {
                free((*key).data.ntp_mac.value as *mut libc::c_void);
            }
            1 => {
...
pub unsafe extern "C" fn KEY_Initialise() {
    keys = ARR_CreateInstance(::std::mem::size_of::<Key>() as libc::c_ulong as libc::c_uint);
    cache_valid = 0 as libc::c_int;
    KEY_Reload();
}
/* ================================================== */
...
/* ================================================== */
unsafe extern "C" fn determine_hash_delay(mut key_id: uint32_t) -> libc::c_int {
    let mut pkt: NTP_Packet = NTP_Packet {
        lvm: 0,
        stratum: 0,
        poll: 0,
        precision: 0,
    ...
    memset(
        &mut pkt as *mut NTP_Packet as *mut libc::c_void,
        0 as libc::c_int,
    ...
    while i < 10 as libc::c_int {
        ...
        KEY_GenerateAuth(
            ...
            &mut pkt.auth_data as *mut [uint8_t; 64] as *mut libc::c_uchar,
            ::std::mem::size_of::<[uint8_t; 64]>() as libc::c_ulong as libc::c_int,
        );
        LCL_ReadRawTime(&mut after);
        diff = UTI_DiffTimespecsToDouble(&mut after, &mut before);
        if i == 0 as libc::c_int || min_diff > diff {
            min_diff = diff
        }
        i += 1
    }
    ...
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"authentication delay for key %u: %d nsecs\x00" as *const u8 as *const libc::c_char,
            key_id,
            nsecs,
        );
    }
...
unsafe extern "C" fn decode_key(mut key: *mut libc::c_char) -> libc::c_int {
    ...
    ) == 0
    {
        ...
        while (j + 1 as libc::c_int) < len {
            ...
            buf[1 as libc::c_int as usize] = *key.offset((j + 1 as libc::c_int) as isize);
            buf[2 as libc::c_int as usize] = '\u{0}' as i32 as libc::c_char;
            *key.offset(i as isize) =
                strtol(buf.as_mut_ptr(), &mut p, 16 as libc::c_int) as libc::c_char;
...
/* ================================================== */
/* Compare two keys */
unsafe extern "C" fn compare_keys_by_id(
...
pub unsafe extern "C" fn KEY_Reload() {
    ...
    if in_0.is_null() {
        LOG_Message(
            LOGS_WARN,
            b"Could not open keyfile %s\x00" as *const u8 as *const libc::c_char,
            key_file,
        );
        return;
    }
    ...
    .is_null()
    {
        ...
        ) == 0
        {
            LOG_Message(
                LOGS_WARN,
                b"Could not parse key at line %u in file %s\x00" as *const u8
            ...
            if key_length == 0 as libc::c_int as libc::c_uint {
                LOG_Message(
                    LOGS_WARN,
                    b"Could not decode key %u\x00" as *const u8 as *const libc::c_char,
                    key.id,
                );
            } else {
                ...
                if hash_id >= 0 as libc::c_int {
                    key.class = NTP_MAC;
                    key.data.ntp_mac.value = Malloc2(
                        key_length as size_t,
                        ::std::mem::size_of::<libc::c_uchar>() as libc::c_ulong,
                    ) as *mut libc::c_uchar;
                    memcpy(
                        key.data.ntp_mac.value as *mut libc::c_void,
                    ...
                    if cmac_key_length != key_length {
                        LOG_Message(
                            LOGS_WARN,
                            b"Invalid length of %s key %u (expected %u bits)\x00" as *const u8
                                as *const libc::c_char,
                            key_type,
                            key.id,
                            (8 as libc::c_int as libc::c_uint).wrapping_mul(cmac_key_length),
                        );
                        continue;
                    } else {
                        key.class = CMAC;
                        key.data.cmac = CMC_CreateInstance(
                            key_type,
                            key_value as *mut libc::c_uchar,
                            key_length,
                        );
                        if !key.data.cmac.is_null() {
                        } else {
                            __assert_fail(
                                b"key.data.cmac\x00" as *const u8 as *const libc::c_char,
                                b"keys.c\x00" as *const u8 as *const libc::c_char,
                                270 as libc::c_int as libc::c_uint,
                                (*::std::mem::transmute::<&[u8; 22], &[libc::c_char; 22]>(
                                    b"void KEY_Reload(void)\x00",
                                ))
                                .as_ptr(),
                            );
                        }
                    ...
                    LOG_Message(
                        LOGS_WARN,
                        b"Unknown hash function or cipher in key %u\x00" as *const u8
                            as *const libc::c_char,
                        key.id,
                    );
                    continue;
                }
                ARR_AppendElement(keys, &mut key as *mut Key as *mut libc::c_void);
            }
        }
    }
    fclose(in_0);
    ...
    more careful! */
    qsort(
        ARR_GetElements(keys),
        ARR_GetSize(keys) as size_t,
        ::std::mem::size_of::<Key>() as libc::c_ulong,
        Some(
            compare_keys_by_id
    ...
    while i < ARR_GetSize(keys) {
        if (*get_key(i.wrapping_sub(1 as libc::c_int as libc::c_uint))).id == (*get_key(i)).id {
            LOG_Message(
                LOGS_WARN,
                b"Detected duplicate key %u\x00" as *const u8 as *const libc::c_char,
                (*get_key(i.wrapping_sub(1 as libc::c_int as libc::c_uint))).id,
            );
        }
        i = i.wrapping_add(1)
    }
...
unsafe extern "C" fn lookup_key(mut id: uint32_t) -> libc::c_int {
    ...
    specimen.id = id;
    where_0 = bsearch(
        &mut specimen as *mut Key as *mut libc::c_void,
        keys_ptr as *const libc::c_void,
        ARR_GetSize(keys) as size_t,
        ::std::mem::size_of::<Key>() as libc::c_ulong,
        Some(
            compare_keys_by_id
    ...
    if where_0.is_null() {
        ...
        pos = where_0.wrapping_offset_from(keys_ptr) as libc::c_long as libc::c_int;
        return pos;
    };
}
/* ================================================== */
...
pub unsafe extern "C" fn KEY_KeyKnown(mut key_id: uint32_t) -> libc::c_int {
    return (get_key_by_id(key_id) != 0 as *mut libc::c_void as *mut Key) as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn KEY_GetAuthDelay(mut key_id: uint32_t) -> libc::c_int {
    let mut key: *mut Key = 0 as *mut Key;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn KEY_GetAuthLength(mut key_id: uint32_t) -> libc::c_int {
    ...
    match (*key).class as libc::c_uint {
        0 => {
            return HSH_Hash(
                (*key).data.ntp_mac.hash_id,
                buf.as_mut_ptr(),
                0 as libc::c_int as libc::c_uint,
                buf.as_mut_ptr(),
                0 as libc::c_int as libc::c_uint,
                buf.as_mut_ptr(),
                ::std::mem::size_of::<[libc::c_uchar; 64]>() as libc::c_ulong as libc::c_uint,
            ) as libc::c_int
        }
        1 => {
            return CMC_Hash(
        ...
        _ => {
            __assert_fail(
                ...
                (*::std::mem::transmute::<&[u8; 32], &[libc::c_char; 32]>(
                    b"int KEY_GetAuthLength(uint32_t)\x00",
                ))
                .as_ptr(),
            );
            return 0 as libc::c_int;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn KEY_CheckKeyLength(mut key_id: uint32_t) -> libc::c_int {
    let mut key: *mut Key = 0 as *mut Key;
    key = get_key_by_id(key_id);
    if key.is_null() {
        return 0 as libc::c_int;
    ...
    match (*key).class as libc::c_uint {
        0 => return ((*key).data.ntp_mac.length >= 10 as libc::c_int) as libc::c_int,
        _ => return 1 as libc::c_int,
    };
}
/* ================================================== */
unsafe extern "C" fn generate_auth(
    mut key: *mut Key,
    ...
    match (*key).class as libc::c_uint {
        0 => {
            return HSH_Hash(
                ...
                (*key).data.ntp_mac.length as libc::c_uint,
                data,
                data_len as libc::c_uint,
                auth,
                auth_len as libc::c_uint,
            ) as libc::c_int
        }
        1 => {
            return CMC_Hash(
...
unsafe extern "C" fn check_auth(
    ...
    return ((if hash_len < trunc_len {
        ...
        && memcmp(
            ...
            auth_len as libc::c_ulong,
        ) == 0) as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn KEY_GenerateAuth(
    mut key_id: uint32_t,
    mut data: *const libc::c_uchar,
    mut data_len: libc::c_int,
    mut auth: *mut libc::c_uchar,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn KEY_CheckAuth(
    mut key_id: uint32_t,
    mut data: *const libc::c_uchar,
    mut data_len: libc::c_int,
    mut auth: *const libc::c_uchar,
