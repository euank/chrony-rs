...
static mut samples: [Sample; 16] = [Sample {
    when: timespec {
        tv_sec: 0,
        tv_nsec: 0,
    },
...
/* ================================================== */
unsafe extern "C" fn estimate_and_set_system(
    mut now: *mut timespec,
    mut offset_provided: libc::c_int,
    mut offset: libc::c_double,
    mut reg_offset: *mut libc::c_double,
    mut dfreq_ppm: *mut libc::c_double,
    mut new_afreq_ppm: *mut libc::c_double,
) {
    let mut agos: [libc::c_double; 16] = [0.; 16]; /* Unused results from regression analyser */
    let mut offsets: [libc::c_double; 16] = [0.; 16]; /* All 9's when printed to log file */
    ...
    if n_samples > 1 as libc::c_int {
        i = 0 as libc::c_int;
        while i < n_samples {
            agos[i as usize] = UTI_DiffTimespecsToDouble(
                &mut (*samples
                    .as_mut_ptr()
                    .offset((n_samples - 1 as libc::c_int) as isize))
                .when,
                &mut (*samples.as_mut_ptr().offset(i as isize)).when,
            );
    ...
    if found_freq != 0 {
        LOG_Message(
            LOGS_INFO,
            b"Making a frequency change of %.3f ppm and a slew of %.6f\x00" as *const u8
        ...
        LOG_Message(
            LOGS_INFO,
            b"Making a slew of %.6f\x00" as *const u8 as *const libc::c_char,
            slew_by,
        );
        REF_SetManualReference(now, slew_by, 0.0f64, skew);
    }
    if !reg_offset.is_null() {
        *reg_offset = b0
    }
    if !dfreq_ppm.is_null() {
        *dfreq_ppm = 1.0e6f64 * freq
    }
    ...
    i = 0 as libc::c_int;
    while i < n_samples {
        samples[i as usize].residual = offsets[i as usize] - (b0 + agos[i as usize] * b1);
        i += 1
    }
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn MNL_AcceptTimestamp(
    mut ts: *mut timespec,
    mut reg_offset: *mut libc::c_double,
    mut dfreq_ppm: *mut libc::c_double,
    mut new_afreq_ppm: *mut libc::c_double,
    ...
    if enabled != 0 {
        ...
        if UTI_IsTimeOffsetSane(ts, 0.0f64) == 0 {
        ...
        if n_samples == 16 as libc::c_int {
            ...
            i = 1 as libc::c_int;
            while i < n_samples {
                samples[(i - 1 as libc::c_int) as usize] = samples[i as usize];
                i += 1
            }
            n_samples -= 1
        }
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn MNL_Enable() {
    enabled = 1 as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn MNL_Disable() {
    enabled = 0 as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn MNL_Reset() {
    n_samples = 0 as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn MNL_IsEnabled() -> libc::c_int {
    return enabled;
}
/* ================================================== */
...
protocol */
#[no_mangle]
pub unsafe extern "C" fn MNL_ReportSamples(
    mut report: *mut RPT_ManualSamplesReport,
    mut max: libc::c_int,
    ...
    while i < n_samples && i < max {
        ...
        (*report.offset(i as isize)).slewed_offset = samples[i as usize].offset;
        (*report.offset(i as isize)).orig_offset = samples[i as usize].orig_offset;
...
#[no_mangle]
pub unsafe extern "C" fn MNL_DeleteSample(mut index: libc::c_int) -> libc::c_int {
    let mut i: libc::c_int = 0;
    let mut now: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
