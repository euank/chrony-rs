...
extern "C" {
    ...
    **********************************************************************
    ...
    #[no_mangle]
    fn RGR_MultipleRegress(
        x1: *mut libc::c_double,
        x2: *mut libc::c_double,
        y: *mut libc::c_double,
        n: libc::c_int,
        b2: *mut libc::c_double,
    ) -> libc::c_int;
    /* Return the median value from an array */
    #[no_mangle]
    fn RGR_FindMedian(x: *mut libc::c_double, n: libc::c_int) -> libc::c_double;
    #[no_mangle]
...
pub unsafe extern "C" fn SST_Initialise() {
    logfileid = if CNF_GetLogStatistics() != 0 {
...
#[no_mangle]
pub unsafe extern "C" fn SST_CreateInstance(
    mut refid: uint32_t,
    mut addr: *mut IPAddr,
    ...
    inst =
        Malloc(::std::mem::size_of::<SST_Stats_Record>() as libc::c_ulong) as *mut SST_Stats_Record;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_SetRefid(
    mut inst: SST_Stats,
    mut refid: uint32_t,
    mut addr: *mut IPAddr,
) {
...
unsafe extern "C" fn prune_register(mut inst: SST_Stats, mut new_oldest: libc::c_int) {
    if new_oldest == 0 {
        return;
    }
    if (*inst).n_samples >= new_oldest {
        ...
        __assert_fail(
            b"inst->n_samples >= new_oldest\x00" as *const u8 as *const libc::c_char,
            b"sourcestats.c\x00" as *const u8 as *const libc::c_char,
            283 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 36], &[libc::c_char; 36]>(
                b"void prune_register(SST_Stats, int)\x00",
            ))
            .as_ptr(),
        );
    }
    ...
    if (*inst).runs_samples > (*inst).n_samples * (2 as libc::c_int - 1 as libc::c_int) {
        (*inst).runs_samples = (*inst).n_samples * (2 as libc::c_int - 1 as libc::c_int)
    }
    if (*inst).n_samples + (*inst).runs_samples <= 64 as libc::c_int * 2 as libc::c_int {
    } else {
        __assert_fail(
            b"inst->n_samples + inst->runs_samples <= MAX_SAMPLES * REGRESS_RUNS_RATIO\x00"
                as *const u8 as *const libc::c_char,
            b"sourcestats.c\x00" as *const u8 as *const libc::c_char,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_AccumulateSample(mut inst: SST_Stats, mut sample: *mut NTP_Sample) {
    let mut n: libc::c_int = 0;
    let mut m: libc::c_int = 0;
    /* Make room for the new sample */
    if (*inst).n_samples > 0 as libc::c_int
        && ((*inst).n_samples == 64 as libc::c_int || (*inst).n_samples == (*inst).max_samples)
    {
    ...
    if (*inst).n_samples != 0
        && UTI_CompareTimespecs(
            &mut *(*inst)
                .sample_times
                .as_mut_ptr()
                .offset((*inst).last_sample as isize),
            &mut (*sample).time,
        ) >= 0 as libc::c_int
    {
        LOG_Message(
            LOGS_WARN,
            b"Out of order sample detected, discarding history for %s\x00" as *const u8
                as *const libc::c_char,
            if !(*inst).ip_addr.is_null() {
                UTI_IPToString((*inst).ip_addr)
            } else {
                UTI_RefidToString((*inst).refid)
            },
        );
        SST_ResetInstance(inst);
    }
    (*inst).last_sample =
        ((*inst).last_sample + 1 as libc::c_int) % (64 as libc::c_int * 2 as libc::c_int);
    n = (*inst).last_sample;
    ...
    if (*inst).n_samples == 0
        || (*inst).peer_delays[n as usize] < (*inst).peer_delays[(*inst).min_delay_sample as usize]
    {
...
unsafe extern "C" fn get_runsbuf_index(mut inst: SST_Stats, mut i: libc::c_int) -> libc::c_int {
    return (((*inst).last_sample + 2 as libc::c_int * 64 as libc::c_int * 2 as libc::c_int
        - (*inst).n_samples
        + i
        + 1 as libc::c_int) as libc::c_uint)
        .wrapping_rem((64 as libc::c_int * 2 as libc::c_int) as libc::c_uint)
        as libc::c_int;
}
/* ================================================== */
/* Return index of the i-th sample in the other buffers */
unsafe extern "C" fn get_buf_index(mut inst: SST_Stats, mut i: libc::c_int) -> libc::c_int {
    return (((*inst).last_sample + 64 as libc::c_int * 2 as libc::c_int - (*inst).n_samples
        + i
        + 1 as libc::c_int) as libc::c_uint)
        .wrapping_rem(64 as libc::c_int as libc::c_uint) as libc::c_int;
}
/* ================================================== */
...
one */
unsafe extern "C" fn convert_to_intervals(
    mut inst: SST_Stats,
    ...
    ts = &mut *(*inst)
        .sample_times
        .as_mut_ptr()
        .offset((*inst).last_sample as isize) as *mut timespec;
    i = -(*inst).runs_samples;
    while i < (*inst).n_samples {
        /* The entries in times_back[] should end up negative */
        *times_back.offset(i as isize) = UTI_DiffTimespecsToDouble(
            &mut *(*inst).sample_times.as_mut_ptr().offset((get_runsbuf_index
                as unsafe extern "C" fn(_: SST_Stats, _: libc::c_int) -> libc::c_int)(
                inst, i
            ) as isize),
            ts,
        );
...
/* ================================================== */
unsafe extern "C" fn find_best_sample_index(
    mut inst: SST_Stats,
    mut times_back: *mut libc::c_double,
) {
    ...
    while i < (*inst).n_samples {
        ...
        if elapsed >= 0.0f64 {
            ...
            __assert_fail(
                b"elapsed >= 0.0\x00" as *const u8 as *const libc::c_char,
                b"sourcestats.c\x00" as *const u8 as *const libc::c_char,
                401 as libc::c_int as libc::c_uint,
                (*::std::mem::transmute::<&[u8; 49], &[libc::c_char; 49]>(
                    b"void find_best_sample_index(SST_Stats, double *)\x00",
                ))
                .as_ptr(),
            );
        }
        root_distance = (*inst).root_dispersions[j as usize]
            + elapsed * (*inst).skew
    ...
    if best_index >= 0 as libc::c_int {
        ...
        __assert_fail(
            b"best_index >= 0\x00" as *const u8 as *const libc::c_char,
            b"sourcestats.c\x00" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn estimate_asymmetry(
    mut times_back: *mut libc::c_double,
    ...
    if abs(*asymmetry_run) < 10 as libc::c_int {
        return 0 as libc::c_int;
    }
    *asymmetry = if -0.5f64 > (if a < 0.5f64 { a } else { 0.5f64 }) {
        -0.5f64
    } else if a < 0.5f64 {
        a
    } else {
...
/* ================================================== */
unsafe extern "C" fn correct_asymmetry(
    mut inst: SST_Stats,
    mut times_back: *mut libc::c_double,
    mut offsets: *mut libc::c_double,
) {
    ...
    if (*inst).fixed_asymmetry == 0.0f64 {
        return;
    }
    ...
    i = 0 as libc::c_int;
    while i < n {
        delays[i as usize] = (*inst).peer_delays
            [get_runsbuf_index(inst, i - (*inst).runs_samples) as usize]
            - min_delay;
        i += 1
    }
    if fabs((*inst).fixed_asymmetry) <= 0.5f64 {
        (*inst).asymmetry = (*inst).fixed_asymmetry
    } else if estimate_asymmetry(
        times_back,
        offsets,
        delays.as_mut_ptr(),
        n,
        &mut (*inst).asymmetry,
        &mut (*inst).asymmetry_run,
    ) == 0
    {
        return;
    }
...
pub unsafe extern "C" fn SST_DoNewRegression(mut inst: SST_Stats) {
    ...
    convert_to_intervals(
        ...
        times_back
            .as_mut_ptr()
            .offset((*inst).runs_samples as isize),
    );
    if (*inst).n_samples > 0 as libc::c_int {
        i = -(*inst).runs_samples;
        while i < (*inst).n_samples {
        ...
        while i < (*inst).n_samples {
            j = get_buf_index(inst, i);
            peer_distances[i as usize] = 0.5f64
        ...
        median_distance = RGR_FindMedian(peer_distances.as_mut_ptr(), (*inst).n_samples);
        sd = (median_distance - min_distance) / 0.7f64;
        sd = if precision > (if sd < min_distance { sd } else { min_distance }) {
            precision
        } else if sd < min_distance {
            sd
        } else {
            min_distance
        };
    ...
    (*inst).regression_ok = RGR_FindBestRegression(
        times_back
            .as_mut_ptr()
            .offset((*inst).runs_samples as isize),
        offsets.as_mut_ptr().offset((*inst).runs_samples as isize),
        weights.as_mut_ptr(),
        (*inst).n_samples,
        (*inst).runs_samples,
        (*inst).min_samples,
    ...
    if (*inst).regression_ok != 0 {
        ...
        (*inst).estimated_frequency_sd = if 1.0e-12f64
        ...
        (*inst).offset_time = (*inst).sample_times[(*inst).last_sample as usize];
        (*inst).estimated_offset_sd = est_intercept_sd;
        (*inst).std_dev = if 1.0e-9f64 > sqrt(est_var) {
            1.0e-9f64
        } else {
            sqrt(est_var)
        };
        (*inst).nruns = nruns;
        (*inst).skew = if 1.0e-12f64
        ...
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"off=%e freq=%e skew=%e n=%d bs=%d runs=%d asym=%f arun=%d\x00" as *const u8
                    as *const libc::c_char,
        ...
        (*inst).nruns = 0 as libc::c_int;
        if (*inst).n_samples > 0 as libc::c_int {
            (*inst).estimated_offset = (*inst).offsets[(*inst).last_sample as usize];
            (*inst).offset_time = (*inst).sample_times[(*inst).last_sample as usize]
        } else {
            (*inst).estimated_offset = 0.0f64;
            UTI_ZeroTimespec(&mut (*inst).offset_time);
    ...
    find_best_sample_index(
        inst,
        times_back.as_mut_ptr().offset(times_back_start as isize),
    );
}
/* ================================================== */
...
time. */
#[no_mangle]
pub unsafe extern "C" fn SST_GetFrequencyRange(
    mut inst: SST_Stats,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_GetSelectionData(
    mut inst: SST_Stats,
    mut now: *mut timespec,
    mut stratum: *mut libc::c_int,
    mut leap: *mut NTP_Leap,
    mut offset_lo_limit: *mut libc::c_double,
    mut offset_hi_limit: *mut libc::c_double,
    mut root_distance: *mut libc::c_double,
    mut std_dev: *mut libc::c_double,
    mut first_sample_ago: *mut libc::c_double,
    mut last_sample_ago: *mut libc::c_double,
    mut select_ok: *mut libc::c_int,
) {
    ...
    *std_dev = (*inst).std_dev;
    sample_elapsed = fabs(UTI_DiffTimespecsToDouble(
        now,
        &mut *(*inst).sample_times.as_mut_ptr().offset(i as isize),
    ));
    offset = (*inst).offsets[i as usize] + sample_elapsed * (*inst).estimated_frequency;
    *root_distance = 0.5f64 * (*inst).root_delays[j as usize]
    ...
    *first_sample_ago = UTI_DiffTimespecsToDouble(
        now,
        &mut *(*inst).sample_times.as_mut_ptr().offset(i as isize),
    );
    i = get_runsbuf_index(inst, (*inst).n_samples - 1 as libc::c_int);
    *last_sample_ago = UTI_DiffTimespecsToDouble(
        now,
        &mut *(*inst).sample_times.as_mut_ptr().offset(i as isize),
    );
    *select_ok = (*inst).regression_ok;
    ...
    if *select_ok == 0
        && (*inst).n_samples < 3 as libc::c_int
    ...
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"n=%d off=%f dist=%f sd=%f first_ago=%f last_ago=%f selok=%d\x00" as *const u8
                as *const libc::c_char,
            (*inst).n_samples,
            offset,
            *root_distance,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_GetTrackingData(
    mut inst: SST_Stats,
    mut ref_time: *mut timespec,
    mut average_offset: *mut libc::c_double,
    ...
    *root_delay = (*inst).root_delays[j as usize];
    elapsed_sample = UTI_DiffTimespecsToDouble(
        &mut (*inst).offset_time,
        &mut *(*inst).sample_times.as_mut_ptr().offset(i as isize),
    );
    *root_dispersion =
        (*inst).root_dispersions[j as usize] + (*inst).skew * elapsed_sample + *offset_sd;
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"n=%d off=%f offsd=%f freq=%e freqsd=%e skew=%e delay=%f disp=%f\x00" as *const u8
                as *const libc::c_char,
            (*inst).n_samples,
            *average_offset,
            *offset_sd,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_SlewSamples(
    mut inst: SST_Stats,
    ...
    let mut prev: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    ...
    while m < (*inst).n_samples {
        ...
        sample = &mut *(*inst).sample_times.as_mut_ptr().offset(i as isize) as *mut timespec;
        prev = *sample;
        UTI_AdjustTimespec(sample, when, sample, &mut delta_time, dfreq, doffset);
    ...
    (*inst).estimated_frequency = ((*inst).estimated_frequency - dfreq) / (1.0f64 - dfreq);
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_AddDispersion(mut inst: SST_Stats, mut dispersion: libc::c_double) {
    let mut m: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    m = 0 as libc::c_int;
    while m < (*inst).n_samples {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_PredictOffset(
    mut inst: SST_Stats,
    mut when: *mut timespec,
) -> libc::c_double {
    let mut elapsed: libc::c_double = 0.;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_MinRoundTripDelay(mut inst: SST_Stats) -> libc::c_double {
    if (*inst).fixed_min_delay > 0.0f64 {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_GetDelayTestData(
    mut inst: SST_Stats,
    mut sample_time: *mut timespec,
    mut last_sample_ago: *mut libc::c_double,
    mut predicted_offset: *mut libc::c_double,
    mut min_delay: *mut libc::c_double,
    mut skew: *mut libc::c_double,
    mut std_dev: *mut libc::c_double,
) -> libc::c_int {
    if (*inst).n_samples < 6 as libc::c_int {
        return 0 as libc::c_int;
    }
    *last_sample_ago = UTI_DiffTimespecsToDouble(sample_time, &mut (*inst).offset_time);
...
#[no_mangle]
pub unsafe extern "C" fn SST_SaveToFile(mut inst: SST_Stats, mut out: *mut FILE) {
    let mut m: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    fprintf(
    ...
    while m < (*inst).n_samples {
        ...
        fprintf(
            ...
            ((*inst).sample_times[i as usize].tv_nsec as libc::c_ulong)
                .wrapping_div(1000 as libc::c_int as libc::c_ulong),
            (*inst).offsets[i as usize],
            (*inst).orig_offsets[j as usize],
...
#[no_mangle]
pub unsafe extern "C" fn SST_LoadFromFile(mut inst: SST_Stats, mut in_0: *mut FILE) -> libc::c_int {
        ...
        while i < (*inst).n_samples {
            ...
            .is_null()
                || sscanf(
                    ...
                    &mut *(*inst).offsets.as_mut_ptr().offset(i as isize) as *mut libc::c_double,
                    &mut *(*inst).orig_offsets.as_mut_ptr().offset(i as isize)
                        as *mut libc::c_double,
                    &mut *(*inst).peer_delays.as_mut_ptr().offset(i as isize)
                        as *mut libc::c_double,
                    &mut *(*inst).peer_dispersions.as_mut_ptr().offset(i as isize)
                        as *mut libc::c_double,
                    &mut *(*inst).root_delays.as_mut_ptr().offset(i as isize)
                        as *mut libc::c_double,
                    &mut *(*inst).root_dispersions.as_mut_ptr().offset(i as isize)
                        as *mut libc::c_double,
                ...
                ) != 10 as libc::c_int
            {
                ...
                (*inst).n_samples = 0 as libc::c_int; /* Load abandoned if any sign of corruption */
                return 0 as libc::c_int;
            } else {
                ...
                (*inst).sample_times[i as usize].tv_nsec =
                    (1000 as libc::c_int as libc::c_ulong).wrapping_mul(usec) as __syscall_slong_t;
                UTI_NormaliseTimespec(&mut *(*inst).sample_times.as_mut_ptr().offset(i as isize));
            }
            i += 1
        }
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_DoSourceReport(
    mut inst: SST_Stats,
    ...
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    let mut last_sample_time: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    if (*inst).n_samples > 0 as libc::c_int {
        ...
        (*report).latest_meas_err =
            0.5f64 * (*inst).root_delays[j as usize] + (*inst).root_dispersions[j as usize];
        ...
        (*report).latest_meas_ago =
            UTI_DiffTimespecsToDouble(now, &mut last_sample_time) as libc::c_ulong
    } else {
        (*report).latest_meas_ago = -(1 as libc::c_int) as uint32_t as libc::c_ulong;
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_DoSourcestatsReport(
    mut inst: SST_Stats,
    ...
    if (*inst).n_samples > 0 as libc::c_int {
        ...
        dspan = UTI_DiffTimespecsToDouble(
            &mut *(*inst)
                .sample_times
                .as_mut_ptr()
                .offset((*inst).last_sample as isize),
        ...
        sample_elapsed = UTI_DiffTimespecsToDouble(
            now,
            &mut *(*inst).sample_times.as_mut_ptr().offset(bi as isize),
        );
        (*report).span_seconds = round(dspan) as libc::c_ulong;
        (*report).est_offset = (*inst).estimated_offset + elapsed * (*inst).estimated_frequency;
        (*report).est_offset_err = (*inst).estimated_offset_sd
            + sample_elapsed * (*inst).skew
            + (0.5f64 * (*inst).root_delays[bj as usize] + (*inst).root_dispersions[bj as usize])
    } else {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SST_GetJitterAsymmetry(mut inst: SST_Stats) -> libc::c_double {
