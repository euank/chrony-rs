 ...
 **********************************************************************
  ...
  */
    #[no_mangle]
    fn RGR_WeightedRegression(x: *mut libc::c_double, y: *mut libc::c_double,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_CreateInstance(mut min_samples: libc::c_int,
 ...
 -> SPF_Instance {
    ...
    filter =
        Malloc(::std::mem::size_of::<SPF_Instance_Record>() as libc::c_ulong)
    ...
    combine_ratio =
        if 0.0f64 >
               (if combine_ratio < 1.0f64 { combine_ratio } else { 1.0f64 }) {
            0.0f64
        } else if combine_ratio < 1.0f64 { combine_ratio } else { 1.0f64 };
    ...
    (*filter).avg_var =
        LCL_GetSysPrecisionAsQuantum() * LCL_GetSysPrecisionAsQuantum();
    (*filter).max_var = max_dispersion * max_dispersion;
    (*filter).combine_ratio = combine_ratio;
    (*filter).samples =
        Malloc2((*filter).max_samples as size_t,
                ::std::mem::size_of::<NTP_Sample>() as libc::c_ulong) as
            *mut NTP_Sample;
    (*filter).selected =
        Malloc2((*filter).max_samples as size_t,
...
unsafe extern "C" fn check_sample(mut filter: SPF_Instance,
                                  mut sample: *mut NTP_Sample)
 -> libc::c_int {
    if (*filter).used <= 0 as libc::c_int { return 1 as libc::c_int }
    if UTI_CompareTimespecs(&mut (*(*filter).samples.offset((*filter).last as
                                                                isize)).time,
                            &mut (*sample).time) >= 0 as libc::c_int {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"filter non-increasing sample time %s\x00" as
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_AccumulateSample(mut filter: SPF_Instance,
                                              mut sample: *mut NTP_Sample)
 -> libc::c_int {
    if check_sample(filter, sample) == 0 { return 0 as libc::c_int }
    (*filter).index += 1;
    (*filter).index %= (*filter).max_samples;
    (*filter).last = (*filter).index;
    if (*filter).used < (*filter).max_samples { (*filter).used += 1 }
    *(*filter).samples.offset((*filter).index as isize) = *sample;
    if 0 as libc::c_int != 0 &&
           log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(LOGS_DEBUG,
                    b"filter sample %d t=%s offset=%.9f peer_disp=%.9f\x00" as
                        *const u8 as *const libc::c_char, (*filter).index,
                    UTI_TimespecToString(&mut (*sample).time),
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_GetLastSample(mut filter: SPF_Instance,
                                           mut sample: *mut NTP_Sample)
 -> libc::c_int {
    if (*filter).last < 0 as libc::c_int { return 0 as libc::c_int }
    *sample = *(*filter).samples.offset((*filter).last as isize);
    return 1 as libc::c_int;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_GetNumberOfSamples(mut filter: SPF_Instance)
 -> libc::c_int {
    return (*filter).used;
}
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_GetAvgSampleDispersion(mut filter: SPF_Instance)
 -> libc::c_double {
...
unsafe extern "C" fn compare_samples(mut a: *const libc::c_void,
                                     mut b: *const libc::c_void)
 -> libc::c_int {
    let mut s1: *const NTP_Sample = 0 as *const NTP_Sample;
    let mut s2: *const NTP_Sample = 0 as *const NTP_Sample;
    s1 =
        &*tmp_sort_samples.offset(*(a as *mut libc::c_int) as isize) as
            *const NTP_Sample;
    s2 =
        &*tmp_sort_samples.offset(*(b as *mut libc::c_int) as isize) as
            *const NTP_Sample;
    if (*s1).offset < (*s2).offset {
        return -(1 as libc::c_int)
    } else { if (*s1).offset > (*s2).offset { return 1 as libc::c_int } }
    return 0 as libc::c_int;
}
/* ================================================== */
unsafe extern "C" fn select_samples(mut filter: SPF_Instance) -> libc::c_int {
    ...
    if (*filter).used < (*filter).min_samples { return 0 as libc::c_int }
    ...
    if (*filter).used > 4 as libc::c_int {
        i = 1 as libc::c_int;
        min_dispersion =
            (*(*filter).samples.offset(0 as libc::c_int as
                                           isize)).peer_dispersion;
        while i < (*filter).used {
            if min_dispersion >
                   (*(*filter).samples.offset(i as isize)).peer_dispersion {
                min_dispersion =
                    (*(*filter).samples.offset(i as isize)).peer_dispersion
            }
            i += 1
        ...
        i = j;
        while i < (*filter).used {
            if (*(*filter).samples.offset(i as isize)).peer_dispersion <=
                   1.5f64 * min_dispersion {
                let fresh0 = j;
                j = j + 1;
    ...
    if j < 4 as libc::c_int {
        /* Select all samples */
        j = 0 as libc::c_int;
        while j < (*filter).used { *selected.offset(j as isize) = j; j += 1 }
    }
    /* And sort their indices by offset */
    tmp_sort_samples = (*filter).samples;
    qsort(selected as *mut libc::c_void, j as size_t,
          ::std::mem::size_of::<libc::c_int>() as libc::c_ulong,
          Some(compare_samples as
    ...
    if j > 2 as libc::c_int {
        from =
            (j as libc::c_double * (1.0f64 - (*filter).combine_ratio) /
                 2.0f64) as libc::c_int;
        from =
            if 1 as libc::c_int >
                   (if from < (j - 1 as libc::c_int) / 2 as libc::c_int {
                        from
                    } else { (j - 1 as libc::c_int) / 2 as libc::c_int }) {
                1 as libc::c_int
            } else if from < (j - 1 as libc::c_int) / 2 as libc::c_int {
                from
            } else { (j - 1 as libc::c_int) / 2 as libc::c_int }
    } else { from = 0 as libc::c_int }
    to = j - from;
    ...
    while i < to {
        *selected.offset(i as isize) =
            (*selected.offset(i as isize) + o) % (*filter).used;
        i += 1
    }
    while i < (*filter).used {
        *selected.offset(i as isize) = -(1 as libc::c_int);
    ...
    while i < (*filter).used {
        if *selected.offset(i as isize) != -(1 as libc::c_int) {
            ...
            *selected.offset(fresh1 as isize) =
                (*selected.offset(i as isize) + (*filter).used - o) %
                    (*filter).used
        }
        i += 1
    }
    if j > 0 as libc::c_int && j <= (*filter).max_samples {
        ...
        __assert_fail(b"j > 0 && j <= filter->max_samples\x00" as *const u8 as
                          *const libc::c_char,
                      b"samplefilt.c\x00" as *const u8 as *const libc::c_char,
                      275 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 33],
                                                &[libc::c_char; 33]>(b"int select_samples(SPF_Instance)\x00")).as_ptr());
    }
    return j;
}
/* ================================================== */
unsafe extern "C" fn combine_selected_samples(mut filter: SPF_Instance,
                                              mut n: libc::c_int,
 ...
 -> libc::c_int {
    ...
    last_sample =
        &mut *(*filter).samples.offset(*(*filter).selected.offset((n -
                                                                       1 as
                                                                           libc::c_int)
                                                                      as
                                                                      isize)
                                           as isize) as *mut NTP_Sample;
    /* Prepare data */
    i = 0 as libc::c_int;
    while i < n {
        sample =
            &mut *(*filter).samples.offset(*(*filter).selected.offset(i as
                                                                          isize)
                                               as isize) as *mut NTP_Sample;
        *(*filter).x_data.offset(i as isize) =
            UTI_DiffTimespecsToDouble(&mut (*sample).time,
                                      &mut (*last_sample).time);
    ...
    if n >= 4 as libc::c_int {
        ...
        i = 0 as libc::c_int;
        while i < n { *(*filter).x_data.offset(i as isize) -= mean_x; i += 1 }
        /* Make a linear fit and use the estimated standard deviation of the
       intercept as dispersion */
        RGR_WeightedRegression((*filter).x_data, (*filter).y_data,
                               (*filter).w_data, n, &mut b0, &mut b1, &mut s2,
                               &mut sb0, &mut sb1);
        var = s2;
        disp = sb0;
    ...
    if var < 1e-20f64 { var = 1e-20f64; disp = sqrt(var) }
    /* Drop the sample if the variance is larger than the maximum */
    if (*filter).max_var > 0.0f64 && var > (*filter).max_var {
        if 0 as libc::c_int != 0 &&
               log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(LOGS_DEBUG,
                        b"filter dispersion too large disp=%.9f max=%.9f\x00"
                            as *const u8 as *const libc::c_char, sqrt(var),
                        sqrt((*filter).max_var));
        }
        return 0 as libc::c_int
    }
    ...
    if (*filter).avg_var_n > 50 as libc::c_int {
        (*filter).avg_var +=
            dof as libc::c_double / (dof as libc::c_double + 50.0f64) *
                (var - (*filter).avg_var)
    } else {
        (*filter).avg_var =
            ((*filter).avg_var * (*filter).avg_var_n as libc::c_double +
    ...
    UTI_AddDoubleToTimespec(&mut (*last_sample).time, mean_x,
                            &mut (*result).time);
    (*result).offset = mean_y;
    (*result).peer_dispersion =
        if disp > mean_peer_dispersion { disp } else { mean_peer_dispersion };
    (*result).root_dispersion =
        if disp > mean_root_dispersion { disp } else { mean_root_dispersion };
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_GetFilteredSample(mut filter: SPF_Instance,
                                               mut sample: *mut NTP_Sample)
 -> libc::c_int {
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_SlewSamples(mut filter: SPF_Instance,
    ...
    if (*filter).last < 0 as libc::c_int { return }
    ...
    } else { last = (*filter).last; first = last }
    i = first;
    while i <= last {
        UTI_AdjustTimespec(&mut (*(*filter).samples.offset(i as isize)).time,
                           when,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SPF_AddDispersion(mut filter: SPF_Instance,
