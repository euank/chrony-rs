...
static mut stages: [stage; 3] = [stage {
    wander: 0.,
    length: 0.,
}; 3];
...
unsafe extern "C" fn get_smoothing(
...
unsafe extern "C" fn update_stages() {
    ...
    while i <= 1 as libc::c_int {
        ...
        l1t[i as usize] = l3t[i as usize] - dir as libc::c_double * smooth_freq / max_wander;
    ...
    stages[0 as libc::c_int as usize].wander = dir as libc::c_double * max_wander;
    ...
    stages[2 as libc::c_int as usize].wander = -dir as libc::c_double * max_wander;
    ...
    while i < 3 as libc::c_int {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"Smooth stage %d wander %e length %f\x00" as *const u8 as *const libc::c_char,
                i + 1 as libc::c_int,
                stages[i as usize].wander,
...
unsafe extern "C" fn update_smoothing(
    mut now: *mut timespec,
    ...
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"Smooth offset %e freq %e\x00" as *const u8 as *const libc::c_char,
            smooth_offset,
            smooth_freq,
        );
    };
...
#[no_mangle]
pub unsafe extern "C" fn SMT_IsEnabled() -> libc::c_int {
    return enabled;
}
#[no_mangle]
pub unsafe extern "C" fn SMT_GetOffset(mut now: *mut timespec) -> libc::c_double {
    let mut offset: libc::c_double = 0.;
    let mut freq: libc::c_double = 0.;
    if enabled == 0 {
        return 0.0f64;
...
pub unsafe extern "C" fn SMT_Activate(mut now: *mut timespec) {
    if enabled == 0 || locked == 0 {
        return;
    }
    LOG_Message(
        LOGS_INFO,
        b"Time smoothing activated%s\x00" as *const u8 as *const libc::c_char,
        if leap_only_mode != 0 {
            b" (leap seconds only)\x00" as *const u8 as *const libc::c_char
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn SMT_Leap(mut now: *mut timespec, mut leap: libc::c_int) {
    /* When the leap-only mode is disabled, the leap second will be accumulated
    in handle_slew() as a normal offset */
    if enabled == 0 || leap_only_mode == 0 {
        return;
    }
    update_smoothing(now, leap as libc::c_double, 0.0f64);
}
#[no_mangle]
pub unsafe extern "C" fn SMT_GetSmoothingReport(
    mut report: *mut RPT_SmoothingReport,
    mut now: *mut timespec,
) -> libc::c_int {
    let mut length: libc::c_double = 0.;
    let mut elapsed: libc::c_double = 0.;
    ...
    if locked == 0 && elapsed >= 0.0f64 {
        ...
        (*report).remaining_time = if elapsed < length {
            (length) - elapsed
        } else {
            0.0f64
        }
