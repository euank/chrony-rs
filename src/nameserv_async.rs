...
/* ================================================== */
unsafe extern "C" fn start_resolving(mut anything: *mut libc::c_void) -> *mut libc::c_void {
    let mut inst: *mut DNS_Async_Instance = anything as *mut DNS_Async_Instance;
    (*inst).status = DNS_Name2IPAddress(
        (*inst).name,
    ...
    (write(
        (*inst).pipe[1 as libc::c_int as usize],
...
/* ================================================== */
unsafe extern "C" fn end_resolving(
    mut fd: libc::c_int,
    ...
    if pthread_join((*inst).thread, 0 as *mut *mut libc::c_void) != 0 {
        LOG_Message(
            LOGS_FATAL,
            b"pthread_join() failed\x00" as *const u8 as *const libc::c_char,
        );
    ...
    i = 0 as libc::c_int;
    while (*inst).status as libc::c_uint == DNS_Success as libc::c_int as libc::c_uint
        && i < 16 as libc::c_int
        && (*inst).addresses[i as usize].family as libc::c_int != 0 as libc::c_int
    {
        i += 1
    }
    (*inst).handler.expect("non-null function pointer")(
        (*inst).status,
        i,
        (*inst).addresses.as_mut_ptr(),
        (*inst).arg,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn DNS_Name2IPAddressAsync(
    mut name: *const libc::c_char,
    mut handler: DNS_NameResolveHandler,
    mut anything: *mut libc::c_void,
) {
    let mut inst: *mut DNS_Async_Instance = 0 as *mut DNS_Async_Instance;
    inst = Malloc(::std::mem::size_of::<DNS_Async_Instance>() as libc::c_ulong)
    ...
    if resolving_threads <= 1 as libc::c_int {
        ...
        __assert_fail(
            b"resolving_threads <= 1\x00" as *const u8 as *const libc::c_char,
            b"nameserv_async.c\x00" as *const u8 as *const libc::c_char,
            120 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 75], &[libc::c_char; 75]>(
                b"void DNS_Name2IPAddressAsync(const char *, DNS_NameResolveHandler, void *)\x00",
            ))
            .as_ptr(),
        );
    }
    if pthread_create(
        ...
        Some(start_resolving as unsafe extern "C" fn(_: *mut libc::c_void) -> *mut libc::c_void),
        inst as *mut libc::c_void,
    ) != 0
    {
        LOG_Message(
            LOGS_FATAL,
            b"pthread_create() failed\x00" as *const u8 as *const libc::c_char,
        );
        exit(1 as libc::c_int);
    }
    SCH_AddFileHandler(
        (*inst).pipe[0 as libc::c_int as usize],
        1 as libc::c_int,
        Some(
            end_resolving
