 ...
 **********************************************************************
  ...
  */
   ...
   provided frequency functions */
    #[no_mangle]
    fn SYS_Generic_CompleteFreqDriver(max_set_freq_ppm: libc::c_double,
...
/* ================================================== */
unsafe extern "C" fn set_leap(mut leap: libc::c_int,
    ...
    txc.modes = 0 as libc::c_int as libc::c_uint;
    applied =
        (SYS_Timex_Adjust(&mut txc, 0 as libc::c_int) == 4 as libc::c_int) as
            libc::c_int;
    ...
    } else if leap < 0 as libc::c_int { sys_status |= 0x20 as libc::c_int }
    ...
    if tai_offset != 0 {
        ...
        if applied != 0 &&
               sys_status & (0x10 as libc::c_int | 0x20 as libc::c_int) == 0 {
            sys_tai_offset +=
                if prev_status & 0x10 as libc::c_int != 0 {
                    1 as libc::c_int
        ...
        if sys_tai_offset != tai_offset {
            ...
            LOG_Message(LOGS_INFO,
                        b"System clock TAI offset set to %d seconds\x00" as
                            *const u8 as *const libc::c_char, tai_offset);
        }
    }
    SYS_Timex_Adjust(&mut txc, 0 as libc::c_int);
    if prev_status != sys_status {
        LOG_Message(LOGS_INFO,
                    b"System clock status %s leap second\x00" as *const u8 as
                        *const libc::c_char,
                    if leap != 0 {
                        if leap > 0 as libc::c_int {
                            b"set to insert\x00" as *const u8 as
                                *const libc::c_char
                        } else {
                            b"set to delete\x00" as *const u8 as
                                *const libc::c_char
                        }
                    } else if applied != 0 {
                        b"reset after\x00" as *const u8 as *const libc::c_char
                    } else {
                        b"set to not insert/delete\x00" as *const u8 as
                            *const libc::c_char
                    });
    };
}
    ...
    let mut txc: timex =
        timex{modes: 0,
              ...
              c2rust_unnamed_c2rust_unnamed_0_c2rust_unnamed_1_c2rust_unnamed_2_c2rust_unnamed_3_c2rust_unnamed_4_c2rust_unnamed_5_c2rust_unnamed_6_c2rust_unnamed_7_c2rust_unnamed_8_c2rust_unnamed_9:
                  [0; 44],};
    if synchronised != 0 {
        if est_error > 16.0f64 { est_error = 16.0f64 }
        if max_error >= 16.0f64 {
            max_error = 16.0f64;
            synchronised = 0 as libc::c_int
        }
    } else { max_error = 16.0f64; est_error = max_error }
    /* On Linux clear the UNSYNC flag only if rtcsync is enabled */
    if CNF_GetRtcSync() == 0 { synchronised = 0 as libc::c_int }
    if synchronised != 0 {
        sys_status &= !(0x40 as libc::c_int)
    } else { sys_status |= 0x40 as libc::c_int }
    txc.modes =
        (0x10 as libc::c_int | 0x8 as libc::c_int | 0x4 as libc::c_int) as
            libc::c_uint;
    txc.status = sys_status;
...
pub unsafe extern "C" fn SYS_Timex_Initialise() {
    SYS_Timex_InitialiseWithFunctions(500.0f64,
                                      1.0f64 /
                                          100 as libc::c_int as
                                              libc::c_double, None, None,
                                      None, 0.0f64, 0.0f64, None, None);
}
/* Initialise with some driver functions replaced with special versions */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SYS_Timex_InitialiseWithFunctions(mut max_set_freq_ppm:
                                                               libc::c_double,
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SYS_Timex_Finalise() { SYS_Generic_Finalise(); }
/* Wrapper for adjtimex()/ntp_adjtime() */
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SYS_Timex_Adjust(mut txc: *mut timex,
                                          mut ignore_error: libc::c_int)
 -> libc::c_int {
    let mut state: libc::c_int = 0;
    state = adjtimex(txc);
    if state < 0 as libc::c_int {
        LOG_Message(if ignore_error != 0 {
                        LOGS_DEBUG as libc::c_int
                    } else { LOGS_FATAL as libc::c_int } as LOG_Severity,
                    b"adjtimex(0x%x) failed : %s\x00" as *const u8 as
                        *const libc::c_char, (*txc).modes,
