...
static mut slew_start: timespec = timespec {
    tv_sec: 0,
    tv_nsec: 0,
};
...
unsafe extern "C" fn handle_step(
    ...
    if change_type as libc::c_uint == LCL_ChangeUnknownStep as libc::c_int as libc::c_uint {
        ...
        update_slew();
    } else if change_type as libc::c_uint == LCL_ChangeStep as libc::c_int as libc::c_uint {
        UTI_AddDoubleToTimespec(&mut slew_start, -doffset, &mut slew_start);
    };
}
/* ================================================== */
unsafe extern "C" fn start_fastslew() {
    if drv_accrue_offset.is_none() {
        return;
    }
    drv_accrue_offset.expect("non-null function pointer")(offset_register, 0.0f64);
    if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
        LOG_Message(
            LOGS_DEBUG,
            b"fastslew offset=%e\x00" as *const u8 as *const libc::c_char,
            offset_register,
        );
    }
    offset_register = 0.0f64;
...
unsafe extern "C" fn stop_fastslew(mut now: *mut timespec) {
    ...
    if drv_get_offset_correction.is_none() || fastslew_active == 0 {
        return;
    }
    /* Cancel the remaining offset */
    drv_get_offset_correction.expect("non-null function pointer")(
        now,
        &mut corr,
...
unsafe extern "C" fn update_slew() {
    ...
    let mut end_of_slew: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    ...
    if drv_accrue_offset.is_some()
        && fabs(corr_freq) >= fastslew_max_rate
    ...
    total_freq = Some(drv_set_freq.expect("non-null function pointer"))
        .expect("non-null function pointer")(total_freq);
    ...
    if fabs(offset_register) < 1.0e-9f64 || offset_register * slew_freq <= 0.0f64 {
        duration = 1.0e4f64
    } else {
        duration = offset_register / slew_freq;
        if duration < 1.0f64 {
            duration = 1.0f64
        } else if duration > 1.0e4f64 {
            duration = 1.0e4f64
        }
    }
    /* Restart timer for the next update */
    UTI_AddDoubleToTimespec(&mut now, duration, &mut end_of_slew);
    slew_timeout_id = SCH_AddTimeout(
        &mut end_of_slew,
        Some(handle_end_of_slew as unsafe extern "C" fn(_: *mut libc::c_void) -> ()),
...
unsafe extern "C" fn apply_step_offset(mut offset: libc::c_double) -> libc::c_int {
    let mut old_time: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    let mut new_time: timespec = timespec {
        tv_sec: 0,
        tv_nsec: 0,
    };
    let mut new_time_tv: timeval = timeval {
        tv_sec: 0,
        tv_usec: 0,
    };
    ...
    if settimeofday(&mut new_time_tv, 0 as *const timezone) < 0 as libc::c_int {
        if 0 as libc::c_int != 0 && log_min_severity as libc::c_int == LOGS_DEBUG as libc::c_int {
            LOG_Message(
                LOGS_DEBUG,
                b"settimeofday() failed\x00" as *const u8 as *const libc::c_char,
            );
        }
...
/* ================================================== */
#[no_mangle]
pub unsafe extern "C" fn SYS_Generic_CompleteFreqDriver(
    ...
    mut sys_set_leap: lcl_SetLeapDriver,
    mut sys_set_sync_status: lcl_SetSyncStatusDriver,
) {
    max_freq = max_set_freq_ppm;
    max_freq_change_delay = max_set_freq_delay * (1.0f64 + max_freq / 1.0e6f64);
    ...
    base_freq = Some(drv_read_freq.expect("non-null function pointer"))
...
pub unsafe extern "C" fn SYS_Generic_Finalise() {
    ...
    SCH_RemoveTimeout(slew_timeout_id);
    slew_timeout_id = 0 as libc::c_int as SCH_TimeoutID;
    Some(drv_set_freq.expect("non-null function pointer")).expect("non-null function pointer")(
        clamp_freq(base_freq),
    );
    LCL_ReadRawTime(&mut now);
    stop_fastslew(&mut now);
}
