...
#[no_mangle]
pub unsafe extern "C" fn ARR_CreateInstance(mut elem_size: libc::c_uint) -> ARR_Instance {
    let mut array: ARR_Instance = 0 as *mut ARR_Instance_Record;
    if elem_size > 0 as libc::c_int as libc::c_uint {
    } else {
        __assert_fail(
            b"elem_size > 0\x00" as *const u8 as *const libc::c_char,
            b"array.c\x00" as *const u8 as *const libc::c_char,
            47 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 46], &[libc::c_char; 46]>(
                b"ARR_Instance ARR_CreateInstance(unsigned int)\x00",
            ))
            .as_ptr(),
        );
    }
    array = Malloc(::std::mem::size_of::<ARR_Instance_Record>() as libc::c_ulong)
...
unsafe extern "C" fn realloc_array(mut array: ARR_Instance, mut min_size: libc::c_uint) {
    if min_size <= (2 as libc::c_int as libc::c_uint).wrapping_mul(min_size) {
    } else {
        __assert_fail(
            b"min_size <= 2 * min_size\x00" as *const u8 as *const libc::c_char,
            b"array.c\x00" as *const u8 as *const libc::c_char,
            69 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 47], &[libc::c_char; 47]>(
                b"void realloc_array(ARR_Instance, unsigned int)\x00",
            ))
            .as_ptr(),
        );
    }
    if (*array).allocated >= min_size
        && (*array).allocated <= (2 as libc::c_int as libc::c_uint).wrapping_mul(min_size)
    {
        return;
    }
    if (*array).allocated < min_size {
        while (*array).allocated < min_size {
            (*array).allocated = if (*array).allocated != 0 {
                (2 as libc::c_int as libc::c_uint).wrapping_mul((*array).allocated)
            } else {
    ...
    (*array).data = Realloc2(
        (*array).data,
        (*array).allocated as size_t,
        (*array).elem_size as size_t,
    );
}
/* Return pointer to a new element added to the end of the array */
#[no_mangle]
pub unsafe extern "C" fn ARR_GetNewElement(mut array: ARR_Instance) -> *mut libc::c_void {
    (*array).used = (*array).used.wrapping_add(1);
    realloc_array(array, (*array).used);
    return ARR_GetElement(
        array,
        (*array).used.wrapping_sub(1 as libc::c_int as libc::c_uint),
    );
}
/* Return element with given index */
#[no_mangle]
pub unsafe extern "C" fn ARR_GetElement(
    mut array: ARR_Instance,
    mut index: libc::c_uint,
) -> *mut libc::c_void {
    if index < (*array).used {
    } else {
        __assert_fail(
            b"index < array->used\x00" as *const u8 as *const libc::c_char,
            b"array.c\x00" as *const u8 as *const libc::c_char,
            94 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 49], &[libc::c_char; 49]>(
                b"void *ARR_GetElement(ARR_Instance, unsigned int)\x00",
            ))
            .as_ptr(),
        );
    }
    return ((*array).data as *mut libc::c_char)
        .offset((index as size_t).wrapping_mul((*array).elem_size as libc::c_ulong) as isize)
        as *mut libc::c_void;
}
/* Return pointer to the internal array of elements */
#[no_mangle]
pub unsafe extern "C" fn ARR_GetElements(mut array: ARR_Instance) -> *mut libc::c_void {
    /* Return a non-NULL pointer when the array has zero size */
    if (*array).data.is_null() {
        if (*array).used == 0 {
        } else {
            __assert_fail(
                b"!array->used\x00" as *const u8 as *const libc::c_char,
                b"array.c\x00" as *const u8 as *const libc::c_char,
                103 as libc::c_int as libc::c_uint,
                (*::std::mem::transmute::<&[u8; 36], &[libc::c_char; 36]>(
                    b"void *ARR_GetElements(ARR_Instance)\x00",
                ))
                .as_ptr(),
            );
        }
...
#[no_mangle]
pub unsafe extern "C" fn ARR_AppendElement(
    mut array: ARR_Instance,
...
#[no_mangle]
pub unsafe extern "C" fn ARR_SetSize(mut array: ARR_Instance, mut size: libc::c_uint) {
    realloc_array(array, size);
    (*array).used = size;
}
/* Return current size of the array */
#[no_mangle]
pub unsafe extern "C" fn ARR_GetSize(mut array: ARR_Instance) -> libc::c_uint {
    return (*array).used;
}
